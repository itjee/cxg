...
export const GET_MANAGER_USERS = gql`
  query GetManagerUsers($limit: Int, $offset: Int, $status: String) {
    managerUsers(limit: $limit, offset: $offset, status: $status) {
      id
      userType
      fullName
      email
...
export function useUpdateManagerUser() {
  ...
  >(UPDATE_MANAGER_USER, {
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['managerUser', data.updateManagerUser.id],
      });
    },
  });
}
...
import { toast } from 'sonner';
import { useManagerUsers, useCreateManagerUser } from '@/features/idam/users/hooks';
...
```typescript
// ✅ 좋음: 일관된 naming
const GET_MANAGER_USERS = gql`...`;
const GET_MANAGER_USER = gql`...`;
...
```typescript
const { data, loading, error } = useManagerUsers();
