...
> - **읽는 시간**: 2-3시간
> - **마지막 업데이트**: 2024년 11월 13일 (GraphQL with Apollo Client 추가)
...
16. [GraphQL with Apollo Client](#graphql-with-apollo-client)
17. [스타일링 시스템](#스타일링-시스템)
...
| GraphQL 클라이언트 | Apollo Client   | 3.11+  | GraphQL 데이터 관리     |
| 스타일             | Tailwind CSS    | v4     | 유틸리티 CSS            |
| UI 컴포넌트        | shadcn/ui       | Latest | 재사용 가능 컴포넌트    |
| 서버 상태          | TanStack Query  | v5     | REST API 데이터 캐싱    |
| 글로벌 상태        | Zustand         | Latest | UI 상태 관리            |
| HTTP 클라이언트    | axios           | Latest | REST API 통신           |
...
└── [domain]/           # 도메인별 feature
    ...
    ├── hooks/          # Apollo Client Hooks (useQuery, useMutation)
    ├── services/       # GraphQL 서비스 (하위 호환용)
...
│   │   │   │   │   ├── fragments.ts              # Tenants Fragment (선택사항)
│   │   │   │   │   └── index.ts
│   │   │   │   │
│   │   │   │   ├── hooks/
│   │   │   │   │   ├── use-tenants.hooks.ts     # Apollo Client Hooks (useQuery, useMutation)
│   │   │   │   │   └── index.ts
│   │   │   │   │
│   │   │   │   ├── services/
│   │   │   │   │   ├── tenants.service.ts       # Apollo 기반 서비스 (하위 호환용)
│   │   │   │   │   └── index.ts
│   │   │   │   │
│   │   │   │   ├── stores/
...
│   │   │
│   │   ├── graphql/                 # ✨ 공통 GraphQL 요소
│   │   │   ├── fragments/          # 공통 Fragment
│   │   │   │   ├── common.ts       # TimestampFields, PaginationInfo 등
│   │   │   │   └── index.ts
│   │   │   └── types/              # GraphQL 공통 타입
│   │   │       └── common.types.ts
│   │   │
│   │   ├── api.ts                   # Axios 인스턴스 (REST API 호환용)
...
CXG 플랫폼은 **Apollo Client**를 사용하여 GraphQL API와 통신합니다. Apollo Client는 자동 캐싱, React Hooks 통합, DevTools 지원 등의 장점을 제공합니다.

### Feature 중심 GraphQL 구조

**핵심 원칙**: GraphQL 쿼리/뮤테이션은 각 Feature 내부에서 관리하며, 공통 요소만 `lib/graphql`에서 관리합니다.
...
└── features/
    └── [domain]/
        └── [entity]/
            ├── graphql/          # Feature별 GraphQL
            │   ├── queries.ts    # 조회 쿼리
            │   ├── mutations.ts  # 생성/수정/삭제
            │   ├── fragments.ts  # Feature 전용 Fragment
            │   └── index.ts
            │
            ├── hooks/
            │   └── use-[entity].hooks.ts  # Apollo Hooks
            │
            └── services/
                └── [entity].service.ts    # 서비스 레이어 (선택)
```
...
### 3. Feature별 GraphQL 쿼리

**파일**: `features/idam/users/graphql/queries.ts`
...
export const GET_USERS = gql`
  query GetUsers($limit: Int, $offset: Int, $status: String) {
...
**파일**: `features/idam/users/graphql/mutations.ts`
...
**파일**: `features/idam/users/graphql/index.ts`
...
### 4. Apollo Client Hooks

**파일**: `features/idam/users/hooks/use-users.hooks.ts`
...
export function useUsers(variables?: GetUsersVariables) {
  return useQuery<{ users: Users[] }, GetUsersVariables>(GET_MANAGER_USERS, {
    variables,
    fetchPolicy: "network-only",
  });
}
...
export function useUser(id: string) {
  return useQuery<{ user: Users }, GetUserVariables>(GET_MANAGER_USER, {
    variables: { id },
    skip: !id,
  });
}
...
export function useCreateUser() {
  ...
  return useMutation<{ createUser: Users }, Error, CreateUserVariables>(
    CREATE_MANAGER_USER,
    {
      onSuccess: () => {
        // 목록 쿼리 refetch
...
export function useUpdateUser() {
  ...
  return useMutation<{ updateUser: Users }, Error, UpdateUserVariables>(
    UPDATE_MANAGER_USER,
    {
      onSuccess: (data) => {
        // 목록과 상세 모두 refetch
        ...
        queryClient.invalidateQueries({
          queryKey: ["user", data.updateUser.id],
        });
      },
    }
  );
...
**파일**: `app/(main)/idam/users/page.tsx`

```typescript
"use client";

import { toast } from "sonner";
import { useUsers, useCreateUser } from "@/features/idam/users/hooks";
import { UsersTable, UsersHeader } from "@/features/idam/users/components";
...
export default function UsersPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <UsersTable data={users} onCreate={handleCreate} />
    </div>
  );
}
```

### 6. Apollo Provider 설정
...
### GraphQL vs REST API 사용 구분

| 기준            | GraphQL (Apollo Client)   | REST API (TanStack Query) |
| --------------- | ------------------------- | ------------------------- |
| **주요 API**    | Manager IDAM, Tenant 관리 | 레거시 API                |
| **새 기능**     | ✅ GraphQL 사용           | ❌                        |
| **캐싱**        | Apollo Cache              | TanStack Query Cache      |
| **관계 데이터** | ✅ 한 번에 조회 가능      | 여러 요청 필요            |
| **타입 안전성** | ✅ gql 태그로 타입 추론   | 수동 타입 정의            |
...
export function UserCard({ user }: { user: UserCardFields }) {
...
### 체크리스트: GraphQL Feature 추가

- [ ] `graphql/` 폴더 생성
- [ ] `queries.ts` 작성 (조회)
- [ ] `mutations.ts` 작성 (생성/수정/삭제)
- [ ] `fragments.ts` 작성 (필요시)
- [ ] Apollo Hooks 작성 (`use-[entity].hooks.ts`)
- [ ] 타입 정의 (Variables 인터페이스)
- [ ] 공통 Fragment 재사용 확인
- [ ] 에러 처리 구현
- [ ] 캐시 정책 설정
- [ ] DevTools로 쿼리 테스트
