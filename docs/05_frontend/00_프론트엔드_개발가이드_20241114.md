...
> - **마지막 업데이트**: 2024년 11월 17일 (Query 필터링 & 상태 관리 완성)
...
export default function UsersPage() {
  ...
  const { currentPage, itemsPerPage, setSearchText, setSelectedStatus, setCurrentPage } =
    useUsersStore();
...
**영향받는 파일:**
- `components/filters/query-bar.tsx` (필터 지우기 버튼 추가)
- `components/filters/query-filter.tsx` (props 추가)
- `components/filters/query-filter-popup.tsx` (배지 및 헤더 개수 수정)
- `features/idam/users/components/users-filter.tsx`
- `app/(main)/idam/users/page.tsx` (필터 처리 로직 구현)
