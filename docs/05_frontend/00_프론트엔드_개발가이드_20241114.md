...
> - **마지막 업데이트**: 2024년 11월 17일 (폼 개선 & 토큰 갱신 수정 - v4.1)
> - **상태**: ✅ 프로덕션 준비 완료 (Users 모듈 기준)
...
### 5. 폼 컴포넌트 (components/users-form.tsx)

**원칙: React Hook Form + Zod 검증 + Hint Icons (Popover 도움말)**

```typescript
/**
 * 사용자 폼 컴포넌트
 * - 생성 모드: 모든 필드 입력
 * - 수정 모드: 기존 데이터 로드 및 username 필드 비활성화
 * - Hint Icons: 각 필드마다 도움말 아이콘 포함
 */
...
export function UsersForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="username"
            {...register("username")}
            disabled={isLoading || isEditing}  // 수정 모드에서는 비활성화
          />
          {errors.username && (
      ...
      <EntityFormButtons onCancel={onCancel} isLoading={isLoading} submitText={isEditing ? "수정" : "등록"} />
    </form>
  );
}
```
...
**주요 특징:**
1. **Hint Icons**: 각 필드 옆에 물음표 아이콘으로 도움말 제공 (Popover)
2. **2열 그리드**: `grid grid-cols-2 gap-4`로 항목 배치
3. **라벨 정렬**: `flex items-center justify-between`으로 힌트 아이콘을 우측에 배치
4. **수정 모드**: initialData 존재 시 username 필드 비활성화, 비밀번호 필드 숨김
5. **폼 데이터 로드**: useEffect에서 조건부로 form.reset() 호출 (생성/수정 모드 구분)
...
- [ ] **components/[entity]-edit.tsx** - 생성/수정 모달
- [ ] **components/[entity]-form.tsx** - 폼 (react-hook-form)
