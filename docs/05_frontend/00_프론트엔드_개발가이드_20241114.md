# CXG 플랫폼 - 프론트엔드 GraphQL 개발 완전가이드

> **Next.js와 React를 사용한 GraphQL 기반 프론트엔드 개발 완전 레퍼런스**
>
> - **대상 독자**: 프론트엔드 개발자, UI/UX 개발자
> - **통신 방식**: GraphQL with Apollo Client (REST API 제거)
> - **마지막 업데이트**: 2024년 11월 20일 (폼 필드 레이아웃 & 비밀번호 관리 개선 - v4.4)
> - **상태**: ✅ 프로덕션 준비 완료 (Users, Roles 모듈 기준)

---

## 목차

1. [빠른 시작](#빠른-시작)
2. [개발 원칙](#개발-원칙)
3. [프로젝트 구조](#프로젝트-구조)
4. [IDAM Users 모듈 완전 가이드](#idam-users-모듈-완전-가이드)
5. [Query 기반 필터링 아키텍처](#query-기반-필터링-아키텍처)
6. [GraphQL & Apollo Client](#graphql--apollo-client)
7. [타입 정의 시스템](#타입-정의-시스템)
8. [데이터 흐름 상세 분석](#데이터-흐름-상세-분석)
9. [상태 관리](#상태-관리)
10. [에러 처리 전략](#에러-처리-전략)
11. [베스트 프랙티스](#베스트-프랙티스)
12. [체크리스트](#체크리스트)

---

## 빠른 시작

### 개발 환경 설정

```bash
cd apps/manager-web
pnpm install
pnpm dev  # http://localhost:8200
```

### 기술 스택

| 계층               | 기술            | 버전   | 목적                    |
| ------------------ | --------------- | ------ | ----------------------- |
| 프레임워크         | Next.js         | 15+    | 메타프레임워크, SSR/SSG |
| 언어               | TypeScript      | 5+     | 타입 안전성             |
| 런타임             | React           | 19+    | UI 라이브러리           |
| GraphQL 클라이언트 | Apollo Client   | 3.11+  | GraphQL API 통신        |
| 스타일             | Tailwind CSS    | v4     | 유틸리티 CSS            |
| UI 컴포넌트        | shadcn/ui       | Latest | 재사용 가능 컴포넌트    |
| 글로벌 상태        | Zustand         | Latest | UI 상태 관리            |
| 폼                 | React Hook Form | Latest | 폼 상태 관리            |
| 검증               | Zod             | Latest | 스키마 검증             |

---

## 개발 원칙

### 1. Feature-Driven Architecture (필수)

모든 기능은 독립적인 Feature 단위로 개발하며, 각 모듈은 동일한 구조를 유지합니다.

```
features/idam/users/                    # Users 엔티티 폴더
├── components/                          # ✅ UI 컴포넌트 (6개 필수)
│   ├── users-columns.tsx               # 테이블 컬럼 정의
│   ├── users-table.tsx                 # 테이블 컴포넌트
│   ├── users-edit.tsx                  # 생성/수정 모달 (Form 통합)
│   ├── users-header.tsx                # 헤더 (제목, 버튼)
│   ├── users-filter.tsx                # 필터 컴포넌트
│   ├── users-stats.tsx                 # 통계 카드
│   └── index.ts
│
├── config/                              # ✅ 폼 설정 (Zod + 필드 구성)
│   ├── users-form-config.ts            # 폼 스키마 및 필드 정의
│   └── index.ts
│
├── graphql/                             # ✅ GraphQL 정의 (쿼리/뮤테이션만)
│   ├── queries.ts                      # GET_USERS, GET_USER
│   ├── mutations.ts                    # CREATE_USER, UPDATE_USER
│   └── index.ts                        # Export 쿼리/뮤테이션만
│
├── hooks/                               # ✅ Apollo Hooks + UI Logic
│   ├── use-users.ts                    # useUsers(), useCreateUser() 등
│   └── index.ts
│
├── types/                               # ✅ 타입 중앙 집중 (단일 소스)
│   ├── users.types.ts                  # User, Variables, Response 모두
│   └── index.ts
│
├── services/                            # (선택) Non-React 컨텍스트
│   ├── users.service.ts
│   └── index.ts
│
├── stores/                              # ✅ UI 상태 관리 (Zustand)
│   ├── users.store.ts
│   └── index.ts
│
└── index.ts                             # 공개 API (Feature Index)
```

### 2. 계층별 책임 (SoC - Separation of Concerns)

```
PAGE/COMPONENT
    ↓ (사용자 입력, 이벤트)
HOOK (useQuery/useMutation)
    ↓ (GraphQL 쿼리/뮤테이션 실행)
GRAPHQL (쿼리/뮤테이션 정의)
    ↓ (HTTP POST)
APOLLO CLIENT (캐싱, 네트워크 최적화)
    ↓ (HTTP)
BACKEND API (GraphQL Server)
    ↓ (데이터 조회/저장)
DATABASE (PostgreSQL)
```

### 3. 타입 정의 원칙

**단 하나의 소스 (Single Source of Truth)**

```
types/users.types.ts에서 모든 타입 정의:
├── User (엔티티)
├── CreateUserInput (입력)
├── UpdateUserInput (입력)
├── UsersQueryVariables (목록 쿼리 변수)
├── UserQueryVariables (단일 쿼리 변수)
├── CreateUserVariables (뮤테이션 변수)
├── UpdateUserVariables (뮤테이션 변수)
└── Response 타입들

graphql/에서는 타입 정의 NOT:
  ❌ graphql/queries.ts에서 타입 정의 금지
  ❌ graphql/mutations.ts에서 타입 정의 금지
  ✅ graphql/에서는 types.ts에서 임포트만
```

### 4. 일관성 우선 (Consistency First)

- **모든 모듈은 동일한 폴더 구조**
- **모든 타입은 types.ts에서 관리**
- **모든 GraphQL은 쿼리/뮤테이션만 정의**
- **모든 Hook은 types.ts에서 타입 임포트**

---

## 프로젝트 구조

### Manager Web 디렉토리 레이아웃

```
apps/manager-web/
├── src/
│   ├── app/                          # Next.js App Router
│   │   ├── (auth)/                  # 인증 라우트
│   │   │   ├── signin/
│   │   │   └── signup/
│   │   │
│   │   └── (main)/                  # 메인 애플리케이션
│   │       ├── dashboard/
│   │       │   └── page.tsx
│   │       │
│   │       ├── idam/                # IDAM 도메인
│   │       │   ├── users/           # 사용자 목록 페이지
│   │       │   │   └── page.tsx
│   │       │   ├── roles/           # 역할 목록 페이지
│   │       │   ├── api-keys/        # API 키 목록 페이지
│   │       │   ├── sessions/        # 세션 목록 페이지
│   │       │   ├── permissions/     # 권한 목록 페이지
│   │       │   └── login-logs/      # 로그인 기록 페이지
│   │       │
│   │       └── layout.tsx
│   │
│   ├── components/                  # 공통 컴포넌트
│   │   ├── ui/                     # shadcn/ui
│   │   └── layouts/                # 레이아웃
│   │
│   ├── features/                    # Feature 모듈 (도메인별)
│   │   ├── auth/                    # 인증
│   │   │
│   │   └── idam/                    # ID&Access Management
│   │       ├── users/               # 사용자 (✅ 완전한 구현)
│   │       │   ├── components/
│   │       │   ├── graphql/
│   │       │   ├── hooks/
│   │       │   ├── types/
│   │       │   ├── stores/
│   │       │   ├── services/
│   │       │   └── index.ts
│   │       │
│   │       ├── roles/               # 역할
│   │       ├── api-keys/            # API 키
│   │       ├── sessions/            # 세션
│   │       ├── permissions/         # 권한
│   │       ├── login-logs/          # 로그인 기록
│   │       │
│   │       └── index.ts
│   │
│   ├── lib/                         # 유틸리티 및 설정
│   │   ├── apollo-client.ts         # Apollo 설정
│   │   └── apollo-provider.tsx      # Apollo Provider
│   │
│   └── providers/                   # Context Providers
│       └── apollo-provider.tsx
│
└── IDAM_DATA_FLOW_MERMAID.md        # 데이터 흐름 Mermaid 다이어그램
```

---

## IDAM Users 모듈 완전 가이드

Users 모듈은 IDAM의 가장 완전한 구현 예시입니다. 다른 모듈(roles, api-keys 등)도 동일한 패턴을 따릅니다.

### 폴더 구조

```
features/idam/users/
├── components/
│   ├── users-columns.tsx      # 테이블 컬럼 (필수)
│   ├── users-table.tsx        # 테이블 (필수)
│   ├── users-edit.tsx         # 생성/수정 모달 (필수, Form 통합)
│   ├── users-header.tsx       # 헤더 (필수)
│   ├── users-filter.tsx       # 필터 (필수)
│   ├── users-stats.tsx        # 통계 (필수)
│   └── index.ts
│
├── config/                     # 폼 설정 (Zod + 필드 구성)
│   ├── users-form-config.ts   # 폼 스키마 및 필드 정의
│   └── index.ts
│
├── graphql/
│   ├── queries.ts             # GET_USERS, GET_USER
│   ├── mutations.ts           # CREATE_USER, UPDATE_USER
│   └── index.ts
│
├── hooks/
│   ├── use-users.ts           # Apollo Hooks
│   └── index.ts
│
├── types/
│   ├── users.types.ts         # 모든 타입 정의 (중앙집중)
│   └── index.ts
│
├── stores/
│   ├── users.store.ts         # UI 상태 (Zustand)
│   └── index.ts
│
├── services/
│   ├── users.service.ts       # Non-React 호출용
│   └── index.ts
│
└── index.ts                   # 공개 API
```

### 1. 타입 정의 (types/users.types.ts)

**원칙: 모든 타입을 여기서 관리합니다**

```typescript
/**
 * 사용자 GraphQL 타입 정의 (camelCase - GraphQL 네이티브)
 * 단일 소스: types.ts에서만 정의, graphql/에서는 임포트만 함
 */

// ===== 엔티티 타입 =====

/**
 * Manager User (ID&Access Management)
 * GraphQL 필드는 camelCase입니다
 *
 * @singular 단일 조회, 단일 엔티티 타입
 */
export interface User {
  id: string;
  userType: "ADMIN" | "MANAGER" | "USER" | "MASTER" | "TENANT" | "SYSTEM" | string;
  fullName: string;
  email: string;
  phone?: string;
  username: string;
  ssoProvider?: string;
  ssoSubject?: string;
  mfaEnabled: boolean;
  status: "ACTIVE" | "INACTIVE" | "LOCKED" | "SUSPENDED";
  lastLoginAt?: string;
  lastLoginIp?: string;
  failedLoginAttempts: number;
  lockedUntil?: string;
  forcePasswordChange: boolean;
  timezone: string;
  locale: string;
  department?: string;
  position?: string;
  createdAt: string;
  updatedAt: string;
}

// ===== 입력 타입 (Input) =====

/**
 * 사용자 생성 입력
 *
 * 주의: password 필드 없음
 * - 백엔드에서 임시 비밀번호를 자동 생성하고 이메일로 전송합니다
 */
export interface CreateUserInput {
  userType: string;
  fullName: string;
  email: string;
  username: string;
  phone?: string;
  department?: string;
  position?: string;
}

export interface UpdateUserInput {
  fullName?: string;
  email?: string;
  phone?: string;
  department?: string;
  position?: string;
  status?: string;
  mfaEnabled?: boolean;
}

// ===== 쿼리 변수 타입 =====

/**
 * 사용자 목록 조회 파라미터 (복수형)
 * GET_USERS 쿼리에서 사용
 */
export interface UsersQueryVariables {
  limit?: number;
  offset?: number;
  userType?: string;
  status?: string;
}

/**
 * 사용자 단일 조회 파라미터 (단수형)
 * GET_USER 쿼리에서 사용
 */
export interface UserQueryVariables {
  id: string;
}

// ===== 뮤테이션 변수 타입 =====

export interface CreateUserVariables {
  input: CreateUserInput;
}

export interface UpdateUserVariables {
  id: string;
  input: UpdateUserInput;
}

// ===== GraphQL 응답 타입 =====

export interface GetUsersResponse {
  users: User[];
}

export interface GetUserResponse {
  user: User;
}

export interface CreateUserResponse {
  createUser: User;
}

export interface UpdateUserResponse {
  updateUser: User;
}
```

### 2. GraphQL 쿼리 정의 (graphql/queries.ts)

**원칙: 쿼리 정의만, 타입은 types.ts에서 임포트**

```typescript
import { gql } from "@apollo/client";
import type { UsersQueryVariables, UserQueryVariables } from "../types/users.types";

/**
 * 사용자 목록 조회 (복수)
 */
export const GET_USERS = gql`
  query GetUsers(
    $limit: Int
    $offset: Int
    $userType: String
    $status: String
  ) {
    users(
      limit: $limit
      offset: $offset
      userType: $userType
      status: $status
    ) {
      id
      userType
      fullName
      email
      status
      createdAt
      updatedAt
    }
  }
`;

/**
 * 사용자 상세 조회 (단수)
 */
export const GET_USER = gql`
  query GetUser($id: ID!) {
    user(id: $id) {
      id
      userType
      fullName
      email
      phone
      username
      status
      createdAt
      updatedAt
    }
  }
`;
```

**주의: 타입 정의는 여기서 하지 않음** ✅

### 3. GraphQL 뮤테이션 정의 (graphql/mutations.ts)

```typescript
import { gql } from "@apollo/client";
import type {
  CreateUserInput,
  UpdateUserInput,
  CreateUserVariables,
  UpdateUserVariables,
} from "../types/users.types";

export const CREATE_USER = gql`
  mutation CreateUser($input: CreateUserInput!) {
    createUser(input: $input) {
      id
      userType
      fullName
      email
      status
      createdAt
    }
  }
`;

export const UPDATE_USER = gql`
  mutation UpdateUser($id: ID!, $input: UpdateUserInput!) {
    updateUser(id: $id, input: $input) {
      id
      userType
      fullName
      email
      status
      updatedAt
    }
  }
`;
```

### 4. GraphQL 인덱스 (graphql/index.ts)

**원칙: 쿼리/뮤테이션만 export, 타입은 절대 export 하지 않음**

```typescript
/**
 * ⚠️ 타입 정의는 ../types/users.types.ts에서 관리됩니다
 * 타입을 사용할 때는 반드시 types.ts에서 직접 임포트하세요
 */

export { GET_USERS, GET_USER } from "./queries";
export { CREATE_USER, UPDATE_USER } from "./mutations";
```

### 5. 폼 컴포넌트 (components/users-edit.tsx)

**원칙: 공통 Form 컴포넌트 + 선언적 폼 설정 (config/users-form-config.ts)**

#### 5-1. 폼 설정 (config/users-form-config.ts)

```typescript
/**
 * 사용자 폼 설정 및 검증 스키마
 * 폼의 구조, 필드, 검증 규칙을 선언적으로 정의
 */

import * as z from "zod";
import type { DynamicFormConfig } from "@/components/features/form-field-config";

export type UserFormData = z.infer<typeof userFormSchema>;

/**
 * 폼 유효성 검증 스키마
 *
 * 주의: 비밀번호 필드 없음
 * - 생성 모드: 백엔드에서 임시 비밀번호 자동 생성 후 이메일로 전송
 * - 수정 모드: 비밀번호 변경 불가 (별도의 비밀번호 재설정 프로세스 사용)
 */
export const userFormSchema = z.object({
  username: z.string().min(3, "사용자명은 3자 이상이어야 합니다"),
  email: z.string().email("유효한 이메일을 입력해주세요"),
  fullName: z.string().min(1, "전체 이름을 입력해주세요"),
  userType: z.string({
    message: "사용자 유형을 선택해주세요",
  }),
  phone: z.string().optional(),
  department: z.string().optional(),
  position: z.string().optional(),
  status: z.enum(["ACTIVE", "INACTIVE", "LOCKED", "SUSPENDED"]).optional(),
});

/**
 * 필드별 힌트 설명
 */
const fieldHints = {
  username: "3자 이상의 고유한 사용자명을 입력하세요",
  email: "유효한 이메일 주소를 입력하세요 (임시 비밀번호가 이메일로 전송됩니다)",
  fullName: "사용자의 실제 이름을 입력하세요",
  userType: "사용자의 역할에 따라 적절한 유형을 선택하세요",
  phone: "연락 가능한 전화번호를 입력하세요",
  department: "소속 부서명을 입력하세요",
  position: "사용자의 직책/직급을 입력하세요",
};

/**
 * 사용자 폼 설정
 */
export const userFormConfig: DynamicFormConfig<UserFormData> = {
  schema: userFormSchema,
  defaultValues: {
    username: "",
    email: "",
    fullName: "",
    userType: "USER",
    phone: "",
    department: "",
    position: "",
    status: "ACTIVE",
  },
  sections: [
    {
      id: "basic-info",
      title: "기본 정보",
      fields: [
        {
          name: "username",
          label: "사용자명",
          type: "text",
          placeholder: "사용자명을 입력하세요 (3자 이상)",
          hint: fieldHints.username,
          required: true,
          disabled: true,  // 생성 후 수정 불가
          grid: "half",
        },
        {
          name: "email",
          label: "이메일",
          type: "email",
          placeholder: "이메일을 입력하세요",
          hint: fieldHints.email,
          required: true,
          grid: "half",
        },
        {
          name: "fullName",
          label: "전체 이름",
          type: "text",
          placeholder: "전체 이름을 입력하세요",
          hint: fieldHints.fullName,
          required: true,
          grid: "full",
        },
        {
          name: "userType",
          label: "사용자 유형",
          type: "select",
          hint: fieldHints.userType,
          required: true,
          grid: "half",
          options: [
            { value: "USER", label: "일반 사용자" },
            { value: "MANAGER", label: "매니저" },
            { value: "ADMIN", label: "관리자" },
          ],
        },
        {
          name: "phone",
          label: "전화",
          type: "text",
          placeholder: "전화번호를 입력하세요",
          hint: fieldHints.phone,
          required: false,
          grid: "half",
        },
        {
          name: "department",
          label: "부서",
          type: "text",
          placeholder: "부서를 입력하세요",
          hint: fieldHints.department,
          required: false,
          grid: "half",
        },
        {
          name: "position",
          label: "직책",
          type: "text",
          placeholder: "직책을 입력하세요",
          hint: fieldHints.position,
          required: false,
          grid: "half",
        },
        {
          name: "status",
          label: "사용자 상태",
          type: "select",
          required: false,
          grid: "half",
          options: [
            { value: "ACTIVE", label: "활성" },
            { value: "INACTIVE", label: "비활성" },
            { value: "LOCKED", label: "잠금" },
            { value: "SUSPENDED", label: "일시정지" },
          ],
        },
      ],
    },
  ],
};
```

#### 5-2. 수정/생성 모달 (components/users-edit.tsx)

```typescript
/**
 * 사용자 생성/수정 Dialog
 * 공통 Form 컴포넌트와 FormDrawer를 사용하여 UI 일관성 유지
 */

import { toast } from "sonner";
import { Form, FormDrawer } from "@/components/features";
import { userFormConfig, type UserFormData } from "../config/users-form-config";
import { useUsersStore } from "../stores/users.store";
import { useUser, useCreateUser, useUpdateUser } from "../hooks";

export function UsersEdit() {
  const { formOpen, selectedId, closeForm } = useUsersStore();

  // 수정 모드일 때만 쿼리 실행
  const { data: userResponse } = useUser(
    formOpen && selectedId ? selectedId : ""
  );
  const editingUser = userResponse?.user;

  // GraphQL 뮤테이션 훅
  const [createUser, { loading: createLoading }] = useCreateUser();
  const [updateUser, { loading: updateLoading }] = useUpdateUser();

  const isLoading = createLoading || updateLoading;
  const isEditing = !!selectedId;

  /**
   * 폼 제출 핸들러
   * 생성/수정 여부에 따라 적절한 뮤테이션 실행
   *
   * 주의: 비밀번호는 폼에서 입력받지 않음
   * - 생성 모드: 백엔드에서 임시 비밀번호 자동 생성 후 이메일로 전송
   * - 수정 모드: 비밀번호 변경 불가
   */
  const handleSubmit = async (formData: UserFormData) => {
    try {
      if (isEditing) {
        // 수정 모드: 기본 정보만 업데이트
        await updateUser({
          variables: {
            id: selectedId,
            input: formData,
          },
        });
        toast.success("사용자가 수정되었습니다");
      } else {
        // 생성 모드: 기본 정보 전송 (백엔드에서 임시 비밀번호 생성)
        await createUser({
          variables: {
            input: formData,
          },
        });
        toast.success("사용자가 생성되었습니다. 임시 비밀번호가 이메일로 전송되었습니다.");
      }
      closeForm();
    } catch (error) {
      const message = error instanceof Error ? error.message : "작업 실패";
      toast.error(
        isEditing
          ? "사용자 수정 실패: " + message
          : "사용자 생성 실패: " + message
      );
    }
  };

  return (
    <FormDrawer
      open={formOpen}
      onOpenChange={closeForm}
      title={isEditing ? "사용자 수정" : "사용자 등록"}
      description={
        isEditing
          ? "사용자 정보를 수정하세요."
          : "새로운 사용자 정보를 입력하세요."
      }
      width="md"
    >
      <Form
        config={userFormConfig}
        initialData={editingUser ? {
          username: editingUser.username,
          email: editingUser.email,
          fullName: editingUser.fullName,
          userType: editingUser.userType,
          phone: editingUser.phone,
          department: editingUser.department,
          position: editingUser.position,
          status: editingUser.status,
        } : undefined}
        onSubmit={handleSubmit}
        onCancel={closeForm}
        isLoading={isLoading}
        submitText={isEditing ? "수정" : "등록"}
      />
    </FormDrawer>
  );
}
```

**주요 특징:**
1. **선언적 폼 구성**: config 파일에서 폼 구조를 일괄 정의
2. **공통 Form 컴포넌트**: 모든 필드 타입 자동 처리 (text, email, select, etc)
3. **FormHintIcon**: 각 필드에 자동으로 도움말 아이콘 추가
4. **조건부 필드**: visible 함수로 필드 표시 여부 제어 (password는 생성 모드에만)
5. **코드 감소**: ~340줄 → ~105줄 (70% 코드량 감소)

### 6. Apollo Hooks (hooks/use-users.ts)

```typescript
import { useQuery, useMutation } from "@apollo/client";
import { GET_USERS, GET_USER, CREATE_USER, UPDATE_USER } from "../graphql";
import type {
  User,
  UsersQueryVariables,
  UserQueryVariables,
  CreateUserVariables,
  UpdateUserVariables,
  GetUsersResponse,
  GetUserResponse,
  CreateUserResponse,
  UpdateUserResponse,
} from "../types/users.types";

// ========== useQuery Hooks (조회) ==========

/**
 * 사용자 목록 조회
 * @param variables - { limit, offset, userType, status }
 * @returns { data.users, loading, error, refetch }
 */
export function useUsers(variables?: UsersQueryVariables) {
  return useQuery<GetUsersResponse, UsersQueryVariables>(GET_USERS, {
    variables: {
      limit: 20,
      offset: 0,
      ...variables,
    },
    fetchPolicy: "cache-and-network",
  });
}

/**
 * 사용자 상세 조회
 */
export function useUser(id: string) {
  return useQuery<GetUserResponse, UserQueryVariables>(GET_USER, {
    variables: { id },
    skip: !id,
  });
}

// ========== useMutation Hooks (생성/수정) ==========

/**
 * 사용자 생성
 */
export function useCreateUser() {
  return useMutation<CreateUserResponse, Error, CreateUserVariables>(
    CREATE_USER,
    {
      refetchQueries: [
        {
          query: GET_USERS,
          variables: { limit: 20, offset: 0 },
        },
      ],
    }
  );
}

/**
 * 사용자 수정
 */
export function useUpdateUser() {
  return useMutation<UpdateUserResponse, Error, UpdateUserVariables>(
    UPDATE_USER
  );
}
```

**중요: 타입을 어디서 임포트하나?**
```typescript
// ✅ 올바른 방법
import type {
  User,
  UsersQueryVariables,
  CreateUserVariables,
} from "../types/users.types";

// ❌ 잘못된 방법 (graphql/index.ts에서 타입을 export 하지 않음)
import type { UsersQueryVariables } from "../graphql";  // 불가능!
```

### 6. 페이지에서 사용 (app/(main)/idam/users/page.tsx)

```typescript
"use client";

import { useState, useEffect } from "react";
import { toast } from "sonner";
import {
  UsersHeader,
  UsersStats,
  UsersFilter,
  UsersTable,
  UsersEdit,
  useUsers,
  useUpdateUser,
  useUsersStore,
  type UsersFilterState,
  type User,
} from "@/features/idam/users";

export default function UsersPage() {
  // 1️⃣ UI 상태 (Zustand Store)
  const { currentPage, itemsPerPage, setSearchText, setSelectedStatus, setCurrentPage, openForm, setSelectedId } =
    useUsersStore();

  // 2️⃣ 쿼리 필터 상태 (팝업에서 수정, 적용 버튼 클릭 시만 서버 쿼리 실행)
  const [queryFilters, setQueryFilters] = useState<Record<string, string[] | null>>({
    status: null,
    userType: null,
  });

  // 3️⃣ 쿼리 텍스트 (자유 검색) - 500ms 디바운스 적용
  const [queryText, setQueryTextLocal] = useState("");
  const [debouncedQueryText, setDebouncedQueryText] = useState("");

  // 3-1️⃣ 쿼리 텍스트 디바운스 (500ms)
  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedQueryText(queryText);
      setSearchText(queryText);
    }, 500);

    return () => clearTimeout(timer);
  }, [queryText, setSearchText]);

  // 4️⃣ GraphQL 쿼리 (Apollo useQuery)
  const {
    data: usersResponse,
    loading,
    refetch,
  } = useUsers({
    limit: itemsPerPage,
    offset: currentPage * itemsPerPage,
    userType: queryFilters.userType ? queryFilters.userType.join(",") : undefined,
    status: queryFilters.status ? queryFilters.status.join(",") : undefined,
    search: debouncedQueryText || undefined,
  });

  // 5️⃣ GraphQL 뮤테이션 (Apollo useMutation)
  const [updateUser, { loading: updating }] = useUpdateUser();

  // 6️⃣ 데이터 처리
  const users = usersResponse?.users || [];

  // 7️⃣ 이벤트 핸들러

  /** 검색 텍스트 변경 (UI 즉시 업데이트, 디바운스 후 검색) */
  const handleQueryTextChange = (text: string) => {
    setQueryTextLocal(text);
  };

  /** 필터 적용 */
  const handleApplyQuery = () => {
    // 필터 적용 시 필요한 추가 작업이 있으면 여기에 작성
    // 현재는 queryFilters 상태 업데이트로 useUsers가 자동으로 refetch됨
  };

  /** 모든 필터 초기화 */
  const handleClearAllFilters = () => {
    setQueryFilters({
      status: null,
      userType: null,
    });
    setSearchText("");
    setQueryTextLocal("");
    setDebouncedQueryText("");
    setCurrentPage(0);  // 페이지 번호 초기화
  };

  /** 사용자 수정 */
  const handleEdit = (user: User) => {
    // 선택된 사용자 ID를 store에 저장
    setSelectedId(user.id);
    // 수정 폼 오픈
    openForm();
  };

  /** 사용자 삭제 (상태 변경) */
  const handleDelete = async (user: any) => {
    if (confirm(`'${user.fullName}' 사용자를 삭제하시겠습니까?`)) {
      try {
        await updateUser({
          variables: {
            id: user.id,
            input: { status: "INACTIVE" },
          },
        });
        toast.success("사용자가 삭제되었습니다");
        refetch();
      } catch (error) {
        toast.error("사용자 삭제에 실패했습니다");
        console.error("Failed to delete user:", error);
      }
    }
  };

  if (loading) return <div className="p-6">로딩 중...</div>;

  return (
    <div className="space-y-6">
      <UsersHeader onRefresh={() => refetch()} />
      <UsersStats data={users} />

      {/* Jira 스타일 Query 필터링 UI */}
      <UsersFilter
        queryText={queryText}
        onQueryTextChange={handleQueryTextChange}
        queryFilters={queryFilters}
        onQueryFiltersChange={setQueryFilters}
        onApplyQuery={handleApplyQuery}
        onClearAllFilters={handleClearAllFilters}
      />

      <UsersTable data={users} isLoading={updating} onEdit={handleEdit} onDelete={handleDelete} />
      <UsersEdit />
    </div>
  );
}
```

**주요 특징:**

1. **Jira 스타일 필터링**:
   - 자유 검색 텍스트 (queryText) + 구조화된 필터 (queryFilters)
   - 필터 팝업에서만 수정 가능 (임시 상태)
   - 적용 버튼 클릭 시만 서버에 쿼리 실행

2. **두 가지 상태 관리**:
   - **UI 상태** (페이지, 페이지 크기): Zustand Store
   - **쿼리 상태** (필터, 검색): Component State

3. **QueryText 디바운스 (500ms)**:
   ```typescript
   // 사용자가 입력하면 UI는 즉시 업데이트
   const handleQueryTextChange = (text: string) => {
     setQueryTextLocal(text);  // UI 즉시 반영
   };

   // 하지만 GraphQL 쿼리는 500ms 후에 실행
   useEffect(() => {
     const timer = setTimeout(() => {
       setDebouncedQueryText(queryText);  // GraphQL 쿼리용
       setSearchText(queryText);            // Store 업데이트
     }, 500);
     return () => clearTimeout(timer);
   }, [queryText, setSearchText]);

   // GraphQL 쿼리는 디바운스된 값 사용
   const { data } = useUsers({
     search: debouncedQueryText || undefined,
   });
   ```
   **목적**: 사용자가 문자를 완성할 때까지 기다렸다가 검색을 실행하여 불필요한 쿼리 감소

4. **Edit Mode 연결**:
   ```typescript
   const handleEdit = (user: User) => {
     setSelectedId(user.id);  // Store에 선택된 사용자 ID 저장
     openForm();              // 폼 모달 오픈
   };
   ```
   - 테이블의 Edit 버튼 → handleEdit 호출
   - selectedId → users-edit.tsx → useUser 쿼리 실행
   - 사용자 데이터 → users-form.tsx의 initialData로 전달

5. **GraphQL 파라미터 변환**:
   ```typescript
   userType: queryFilters.userType ? queryFilters.userType.join(",") : undefined,
   status: queryFilters.status ? queryFilters.status.join(",") : undefined,
   ```
   배열을 쉼표로 구분된 문자열로 변환

---

## Query 기반 필터링 아키텍처

### 개요

Users 모듈에서 구현된 **Query 기반 필터링**은 Jira의 필터링 방식을 차용합니다.

```
[🔍 검색어...] [필터 3] [✕ 필터 지우기] [⋯]
```

### 필터링의 두 가지 요소

```typescript
// 1️⃣ 자유 검색 텍스트 (쿼리 텍스트)
queryText: string                              // "john doe"

// 2️⃣ 구조화된 필터 (쿼리 필터)
queryFilters: Record<string, string[] | null>  // { status: ["ACTIVE", "LOCKED"], userType: ["ADMIN"] }
```

### 데이터 흐름

```
사용자 입력
    ↓
┌─────────────────────────────────────┐
│ QueryBar (UI)                       │
│ - 검색 입력                         │
│ - 필터 버튼 클릭                    │
│ - 활성 필터 개수 배지              │
│ - 필터 지우기 버튼                 │
└─────────────┬───────────────────────┘
              ↓ onClick
┌─────────────────────────────────────┐
│ QueryFilterPopup (모달)             │
│ - Jira 스타일 좌우 분할 레이아웃   │
│ - 좌측: 필터 항목 목록              │
│ - 우측: 체크박스 멀티 선택          │
│ - 임시 상태 (tempQueryFilters)     │
└─────────────┬───────────────────────┘
              ↓ 적용 버튼
┌─────────────────────────────────────┐
│ Page Component                      │
│ - queryFilters 상태 업데이트       │
│ - queryText 업데이트                │
└─────────────┬───────────────────────┘
              ↓
┌─────────────────────────────────────┐
│ Apollo useQuery (자동 refetch)      │
│ - 변수 변환 (배열 → 쉼표 구분 문자) │
└─────────────┬───────────────────────┘
              ↓
┌─────────────────────────────────────┐
│ GraphQL Query                       │
│ { users(status, userType, search) }│
└─────────────┬───────────────────────┘
              ↓
┌─────────────────────────────────────┐
│ Backend API (GraphQL Server)        │
│ - SQL WHERE 절 생성                 │
└─────────────┬───────────────────────┘
              ↓
┌─────────────────────────────────────┐
│ Database Query Result               │
│ - 필터링된 사용자 목록              │
└─────────────────────────────────────┘
```

### 1️⃣ 상태 관리 구조

```typescript
// ===== Page Component (app/(main)/idam/users/page.tsx) =====

// UI 상태 (Zustand Store)
const { currentPage, itemsPerPage, setCurrentPage, setSearchText } = useUsersStore();

// 쿼리 상태 (Component State)
const [queryText, setQueryTextLocal] = useState("");
const [queryFilters, setQueryFilters] = useState<Record<string, string[] | null>>({
  status: null,
  userType: null,
});
```

**상태 분리 이유:**
- **UI 상태**: 페이지 번호, 페이지 크기는 전체 앱에서 공유 (Zustand)
- **쿼리 상태**: 필터는 페이지별로 독립적 (Component State)

### 2️⃣ 필터 팝업의 임시 상태 관리

```typescript
// ===== QueryFilterPopup Component (query-filter-popup.tsx) =====

// 팝업 내부에서만 사용하는 임시 상태
const [tempQueryFilters, setTempQueryFilters] = useState<Record<string, string[] | null>>(
  queryFilters
);

// 팝업 열 때 부모 상태 복사
const handleOpenChange = useCallback((newOpen: boolean) => {
  if (newOpen) {
    setTempQueryFilters(queryFilters);  // 부모 상태 복사
  }
  onOpenChange(newOpen);
}, [queryFilters, onOpenChange]);

// 체크박스 선택 (임시 상태만 업데이트)
const handleCheckboxChange = useCallback(
  (values: string[]) => {
    setTempQueryFilters({
      ...tempQueryFilters,
      [selectedItemKey]: values.length > 0 ? values : null,
    });
  },
  [selectedItemKey, tempQueryFilters]
);

// 적용 버튼 (임시 상태 → 부모 상태)
const handleApply = useCallback(() => {
  onQueryFiltersChange(tempQueryFilters);  // 부모에 반영
  onOpenChange(false);  // 팝업 닫기
}, [tempQueryFilters, onQueryFiltersChange, onOpenChange]);
```

**임시 상태의 목적:**
- 팝업에서만 필터 수정 가능
- 취소하면 변경사항 무시
- 적용 버튼 클릭 시만 부모 상태 업데이트
- 팝업 내 변경이 서버 쿼리를 즉시 트리거하지 않음

### 3️⃣ GraphQL 변수 변환

```typescript
// Page에서 GraphQL에 전달하기 전에 변환 필요
const { data } = useUsers({
  limit: itemsPerPage,
  offset: currentPage * itemsPerPage,
  // ❌ 잘못됨: [백엔드는 쉼표 구분 문자열 기대]
  // userType: queryFilters.userType,

  // ✅ 올바름: 배열을 문자열로 변환
  userType: queryFilters.userType ? queryFilters.userType.join(",") : undefined,
  status: queryFilters.status ? queryFilters.status.join(",") : undefined,

  search: queryText || undefined,
});
```

**변환 규칙:**
```typescript
// Frontend (배열)
{ status: ["ACTIVE", "LOCKED"] }

// GraphQL 변수 (문자열)
{ status: "ACTIVE,LOCKED" }

// Backend (문자열 파싱)
status in ['ACTIVE', 'LOCKED']
```

### 4️⃣ 필터 초기화

```typescript
const handleClearAllFilters = () => {
  // 1. 모든 필터 초기화
  setQueryFilters({
    status: null,
    userType: null,
  });

  // 2. 검색 텍스트 초기화
  setSearchText("");
  setQueryTextLocal("");

  // 3. 페이지 번호 초기화 (중요!)
  setCurrentPage(0);
};
```

**페이지 초기화가 중요한 이유:**
- 필터를 벗으면 전체 데이터 세트 반환
- 이전 페이지 번호(예: page=2)가 유효하지 않을 수 있음
- 그래서 항상 page=0(첫 페이지)로 초기화

### 5️⃣ 공통 필터 컴포넌트 (components/filters/)

이제 `@/components/filters`에서 재사용 가능한 필터 컴포넌트를 제공합니다:

```typescript
// ===== Query 기반 필터링 컴포넌트 =====

// 1. QueryBar - 검색 + 필터 버튼 UI
import { QueryBar } from "@/components/filters";

<QueryBar
  queryText={queryText}
  onQueryTextChange={setQueryText}
  queryFilters={queryFilters}
  onQueryFilterClick={() => setFilterOpen(true)}
  onClearAllFilters={handleClear}
/>

// 2. QueryFilter - QueryBar + QueryFilterPopup (복합)
import { QueryFilter } from "@/components/filters";

<QueryFilter
  queryText={queryText}
  onQueryTextChange={setQueryText}
  queryFilters={queryFilters}
  onQueryFiltersChange={setQueryFilters}
  onApply={handleApply}
  onClearAllFilters={handleClear}
  filterItems={[
    { key: "status", label: "상태", options: [...] },
    { key: "userType", label: "사용자 타입", options: [...] },
  ]}
/>

// 3. QueryFilterPopup - Jira 스타일 필터 팝업 (모달)
import { QueryFilterPopup } from "@/components/filters";

<QueryFilterPopup
  open={filterOpen}
  onOpenChange={setFilterOpen}
  queryFilters={queryFilters}
  onQueryFiltersChange={setQueryFilters}
  onApply={handleApply}
  items={filterItems}
/>
```

---

## GraphQL & Apollo Client

### 1. Apollo Client 설정 (lib/apollo-client.ts)

```typescript
import { ApolloClient, InMemoryCache, HttpLink, ApolloLink, from, gql } from '@apollo/client';
import { onError } from '@apollo/client/link/error';
import { setContext } from '@apollo/client/link/context';

const GRAPHQL_ENDPOINT = process.env.NEXT_PUBLIC_GRAPHQL_URL || 'http://localhost:8100/graphql/manager';

/**
 * 토큰 갱신 뮤테이션 정의
 * ⚠️ 중요: 'auth' 네임스페이스를 포함하지 않음 (백엔드 GraphQL 구조에 맞춤)
 */
const REFRESH_TOKEN_MUTATION = gql`
  mutation RefreshToken($input: RefreshTokenInput!) {
    refreshToken(input: $input) {
      accessToken
      refreshToken
      tokenType
      expiresIn
    }
  }
`;

/**
 * 토큰 갱신 함수
 * Refresh Token으로 새로운 Access Token을 발급받습니다.
 */
async function refreshAccessToken(): Promise<{ accessToken: string; refreshToken: string } | null> {
  try {
    if (typeof window === 'undefined') return null;

    const refreshToken = localStorage.getItem('refresh_token');
    if (!refreshToken) {
      console.warn('No refresh token available');
      return null;
    }

    const response = await fetch(GRAPHQL_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        query: `
          mutation RefreshToken($input: RefreshTokenInput!) {
            refreshToken(input: $input) {
              accessToken
              refreshToken
              tokenType
              expiresIn
            }
          }
        `,
        variables: {
          input: { refreshToken },
        },
      }),
    });

    const data = await response.json();

    if (data.errors) {
      console.error('토큰 갱신 오류:', data.errors);
      return null;
    }

    // ✅ 올바른 응답 경로: data.data.refreshToken (auth 네임스페이스 제거됨)
    const { accessToken, refreshToken: newRefreshToken } = data.data.refreshToken;

    // 새 토큰 저장
    localStorage.setItem('access_token', accessToken);
    localStorage.setItem('refresh_token', newRefreshToken);

    return { accessToken, refreshToken: newRefreshToken };
  } catch (error) {
    console.error('토큰 갱신 실패:', error);
    return null;
  }
}

// 1️⃣ 인증 Link
const authLink = setContext((_, { headers }) => {
  let token: string | null = null;

  if (typeof window !== "undefined") {
    token = localStorage.getItem("access_token");
    if (!token) {
      token = document.cookie
        .split("; ")
        .find((row) => row.startsWith("access_token="))
        ?.split("=")[1] ?? null;
    }
  }

  return {
    headers: {
      ...headers,
      authorization: token ? `Bearer ${token}` : "",
    },
  };
});

// 2️⃣ 에러 처리 Link
const errorLink = onError(({ graphQLErrors, networkError, operation, forward }) => {
  if (graphQLErrors) {
    graphQLErrors.forEach(({ message, locations, path, extensions }) => {
      console.error(
        `[GraphQL error]: Message: ${message}, Location: ${locations}, Path: ${path}`,
        extensions
      );

      // 인증 실패 에러 처리
      if (extensions?.code === 'UNAUTHENTICATED' || message?.includes('Authentication required')) {
        if (typeof window !== "undefined") {
          localStorage.removeItem("access_token");
          localStorage.removeItem("refresh_token");
          window.location.href = "/signin";
        }
      }
    });
  }

  if (networkError) {
    console.error(`[Network error]: ${networkError}`);

    // HTTP 401 에러 처리 - 토큰 갱신 시도
    if ('statusCode' in networkError && networkError.statusCode === 401) {
      if (typeof window !== "undefined") {
        (async () => {
          const newTokens = await refreshAccessToken();

          if (newTokens) {
            console.log('Token refreshed, retrying request');
            return forward(operation);
          } else {
            console.log('Token refresh failed, redirecting to login');
            localStorage.removeItem("access_token");
            localStorage.removeItem("refresh_token");
            window.location.href = "/signin";
          }
        })();
      }
    }
  }
});

// 3️⃣ HTTP Link
const httpLink = new HttpLink({
  uri: GRAPHQL_ENDPOINT,
  credentials: "include",
});

// 4️⃣ Apollo Client
export const apolloClient = new ApolloClient({
  link: from([errorLink, authLink, httpLink]),
  cache: new InMemoryCache({
    typePolicies: {
      Query: {
        fields: {
          users: {
            keyArgs: ['userType', 'status'],
            merge(existing = [], incoming) {
              return incoming;
            },
          },
        },
      },
    },
  }),
  defaultOptions: {
    watchQuery: {
      fetchPolicy: "cache-and-network",
      errorPolicy: "all",
    },
    query: {
      fetchPolicy: "network-only",
      errorPolicy: "all",
    },
    mutate: {
      errorPolicy: "all",
    },
  },
  devtools: {
    enabled: process.env.NODE_ENV === 'development',
  },
});
```

**토큰 갱신 GraphQL 쿼리 주의사항:**
```typescript
// ❌ 잘못됨: 'auth' 네임스페이스 포함
mutation RefreshToken($input: RefreshTokenInput!) {
  auth {
    refreshToken(input: $input) {
      accessToken
      refreshToken
    }
  }
}

// ✅ 올바름: 네임스페이스 제거
mutation RefreshToken($input: RefreshTokenInput!) {
  refreshToken(input: $input) {
    accessToken
    refreshToken
  }
}
```

### 2. Apollo Provider (lib/apollo-provider.tsx)

```typescript
"use client";

import { ApolloProvider as Provider } from "@apollo/client";
import { apolloClient } from "./apollo-client";

interface ApolloProviderProps {
  children: React.ReactNode;
}

export function ApolloProvider({ children }: ApolloProviderProps) {
  return <Provider client={apolloClient}>{children}</Provider>;
}
```

### 3. 앱 레이아웃에서 Apollo 적용 (app/layout.tsx)

```typescript
import { ApolloProvider } from "@/lib/apollo-provider";

export default function RootLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return (
    <html lang="ko">
      <body>
        <ApolloProvider>{children}</ApolloProvider>
      </body>
    </html>
  );
}
```

---

## 타입 정의 시스템

### 원칙: 단일 소스 (Single Source of Truth)

모든 타입 정의는 **types/module.types.ts 파일 하나**에서만 관리됩니다.

```
types/users.types.ts는 다음을 모두 포함합니다:
├── 엔티티 (User)
├── 입력 (CreateUserInput, UpdateUserInput)
├── 쿼리 변수 (UsersQueryVariables, UserQueryVariables)
├── 뮤테이션 변수 (CreateUserVariables, UpdateUserVariables)
└── 응답 (GetUsersResponse, GetUserResponse, ...)

graphql/queries.ts 또는 mutations.ts에서:
├── ✅ types.ts에서 타입 임포트 가능
└── ❌ 새로운 타입 정의 금지
```

### 타입 명명 규칙

```typescript
// 엔티티 (단수형)
export interface User { ... }
export interface Role { ... }
export interface Permission { ... }

// 입력 (단수형, Input suffix)
export interface CreateUserInput { ... }
export interface UpdateUserInput { ... }
export interface CreateRoleInput { ... }

// 쿼리 변수 (복수형 = 목록, 단수형 = 단일)
export interface UsersQueryVariables { ... }        // 목록 조회
export interface UserQueryVariables { ... }         // 단일 조회
export interface RolesQueryVariables { ... }
export interface RoleQueryVariables { ... }

// 뮤테이션 변수 (단수형, Variables suffix)
export interface CreateUserVariables { ... }
export interface UpdateUserVariables { ... }
export interface DeleteUserVariables { ... }

// 응답 (Response suffix, camelCase)
export interface GetUsersResponse { ... }
export interface GetUserResponse { ... }
export interface CreateUserResponse { ... }
```

---

## 데이터 흐름 상세 분석

### 시나리오 1: 사용자 목록 조회

```
1️⃣ PAGE (app/(main)/idam/users/page.tsx)
   └─ useUsers({ limit: 20, offset: 0 })

2️⃣ HOOK (hooks/use-users.ts)
   └─ useQuery<GetUsersResponse, UsersQueryVariables>(GET_USERS, ...)

3️⃣ GRAPHQL (graphql/queries.ts)
   └─ GET_USERS = gql`query GetUsers($limit: Int, ...) { ... }`

4️⃣ APOLLO CLIENT
   └─ HTTP POST to /graphql
      Request: { operationName: "GetUsers", variables: { ... } }

5️⃣ BACKEND (Python Strawberry)
   └─ users(limit, offset) → ManagerUser[]

6️⃣ DATABASE
   └─ SELECT * FROM manager_users LIMIT 20 OFFSET 0

7️⃣ RESPONSE (역순)
   └─ users: [ { id, userType, fullName, ... } ]

8️⃣ HOOK
   └─ data: { users: User[] }

9️⃣ PAGE
   └─ <UsersTable data={users} />
```

### 타입 변환 흐름

```
Frontend (camelCase)
├─ UsersQueryVariables: { limit: 20, offset: 0 }
├─ User: { id, userType, fullName, ... }

GraphQL (camelCase - 네이티브)
├─ query GetUsers($limit: Int, $offset: Int)
├─ users: [ { id, userType, fullName } ]

Backend (Python snake_case)
├─ user_type, full_name, created_at
├─ Strawberry 자동 변환: user_type → userType

Database (snake_case)
├─ user_type, full_name, created_at
```

---

## 상태 관리

### Zustand Store (UI 상태 분리)

**원칙: 서버 상태(API 데이터)는 Apollo에서, UI 상태(필터, 페이지, 폼)는 Zustand에서 관리**

```typescript
// features/idam/users/stores/users.store.ts
import { create } from "zustand";

interface UsersStore {
  // ===== UI 상태 =====
  currentPage: number;
  itemsPerPage: number;
  searchText: string;
  selectedStatus: string | null;
  formOpen: boolean;
  selectedId: string | null;
  sorting: Array<{ id: string; desc: boolean }>;

  // ===== 액션 =====
  setCurrentPage: (page: number) => void;
  setItemsPerPage: (size: number) => void;
  setSearchText: (text: Updater<string>) => void;
  setSelectedStatus: (status: string | null) => void;
  setSelectedId: (id: string | null) => void;
  openForm: (id?: string) => void;
  closeForm: () => void;
  setSorting: (sorting: Updater<Array<{ id: string; desc: boolean }>>) => void;
  resetFilters: () => void;
  reset: () => void;
}

const initialState = {
  currentPage: 0,
  itemsPerPage: 20,
  searchText: "",
  selectedStatus: null,
  formOpen: false,
  selectedId: null,
  sorting: [],
};

export const useUsersStore = create<UsersStore>((set) => ({
  ...initialState,

  setCurrentPage: (page) => set({ currentPage: page }),
  setItemsPerPage: (size) => set({ itemsPerPage: size, currentPage: 0 }),
  setSearchText: (text) =>
    set((state) => ({
      searchText: typeof text === 'function' ? text(state.searchText) : text,
      currentPage: 0,
    })),
  setSelectedStatus: (status) =>
    set({
      selectedStatus: status,
      currentPage: 0,
    }),
  setSelectedId: (id) => set({ selectedId: id }),
  openForm: (id) => set({ formOpen: true, selectedId: id || null }),
  closeForm: () => set({ formOpen: false, selectedId: null }),
  setSorting: (sorting) =>
    set((state) => ({
      sorting: typeof sorting === 'function' ? sorting(state.sorting) : sorting,
    })),
  resetFilters: () =>
    set({
      searchText: "",
      selectedStatus: null,
      sorting: [],
      currentPage: 0,
    }),
  reset: () => set(initialState),
}));
```

**페이지에서 사용:**

```typescript
// 1. 목록 조회
const { currentPage, itemsPerPage, selectedStatus } = useUsersStore();
const { data } = useUsers({
  limit: itemsPerPage,
  offset: currentPage * itemsPerPage,
  status: selectedStatus,
});

// 2. 수정 모드 활성화
const { setSelectedId, openForm } = useUsersStore();
const handleEdit = (user: User) => {
  setSelectedId(user.id);  // 선택된 사용자 ID 저장
  openForm();               // 폼 모달 오픈
};

// 3. 수정 폼에서 데이터 로드
const { selectedId } = useUsersStore();
const { data: editingUserData } = useUser(selectedId);
```

**주요 메서드 설명:**
- `setSelectedId(id)`: 선택된 사용자 ID를 저장 (Edit 모드 진입 시)
- `openForm(id?)`: 폼 모달을 열고 선택된 ID 설정
- `closeForm()`: 폼 모달을 닫고 selectedId 초기화

---

## 에러 처리 전략

```typescript
const { data, loading, error } = useUsers();

if (error) {
  // GraphQL 에러
  if (error.graphQLErrors && error.graphQLErrors.length > 0) {
    error.graphQLErrors.forEach(({ message, extensions }) => {
      console.error(`[${extensions?.code}] ${message}`);

      if (extensions?.code === "AUTHENTICATION_REQUIRED") {
        window.location.href = "/signin";
      } else if (extensions?.code === "FORBIDDEN") {
        toast.error("권한이 없습니다");
      } else {
        toast.error(message);
      }
    });
  }

  // 네트워크 에러
  if (error.networkError) {
    toast.error("네트워크 연결 실패");
  }
}
```

---

## 베스트 프랙티스

### 1. ✅ 타입 임포트 위치

```typescript
// ✅ 올바름: types.ts에서만 임포트
import type {
  User,
  UsersQueryVariables,
  CreateUserVariables,
} from "../types/users.types";

import { GET_USERS, CREATE_USER } from "../graphql";

// ❌ 틀림: graphql/index.ts에서 타입 임포트
import type { UsersQueryVariables } from "../graphql";  // 불가능!
```

### 2. ✅ GraphQL 쿼리 최적화

```typescript
// ❌ 나쁨: 불필요한 필드
export const GET_USER = gql`
  query GetUser($id: ID!) {
    user(id: $id) {
      id
      fullName
      email
      phone
      ssoProvider
      timezone
      locale
      # ... 모든 필드
    }
  }
`;

// ✅ 좋음: 필요한 필드만
export const GET_USER = gql`
  query GetUser($id: ID!) {
    user(id: $id) {
      id
      fullName
      email
      status
      createdAt
    }
  }
`;
```

### 3. ✅ Hook 사용 규칙

```typescript
// ✅ 권장: 컴포넌트에서 Hook 사용
function UsersPage() {
  const { data } = useUsers();
}

// ⚠️ Non-React 컨텍스트: Service 사용
// (API Routes, Utils 등에서만)
import { usersService } from "./services/users.service";
const users = await usersService.listUsers();
```

### 4. ✅ 에러 메시지

```typescript
// ❌ 나쁨: 기술적 에러 노출
toast.error(`GraphQL Error: ${error.message}`);

// ✅ 좋음: 사용자 친화적 메시지
toast.error("사용자 생성에 실패했습니다. 다시 시도해주세요.");
```

### 5. ✅ 캐시 무효화

```typescript
// refetchQueries를 사용해 관련 쿼리 자동 갱신
const [createUser] = useCreateUser();

await createUser({
  variables: { input },
  refetchQueries: [
    {
      query: GET_USERS,
      variables: { limit: 20, offset: 0 },
    },
  ],
});
```

---

## 체크리스트

### 새로운 Entity 개발 체크리스트

#### 1단계: 폴더 및 타입 구조

- [ ] Feature 폴더 생성 (`features/idam/[entity]/`)
- [ ] **types/[entity].types.ts** - 모든 타입 정의
  - [ ] Entity 인터페이스
  - [ ] Input 인터페이스
  - [ ] QueryVariables 인터페이스 (캐멜 케이스)
  - [ ] Mutation Variables 인터페이스
  - [ ] Response 인터페이스

#### 2단계: GraphQL 정의

- [ ] **graphql/queries.ts** - 쿼리 정의
  - [ ] types.ts에서 타입 임포트 ✅
  - [ ] 필요한 필드만 선택 (과다 요청 금지)
  - [ ] 필터 파라미터 지원 (search, status, userType 등)
- [ ] **graphql/mutations.ts** - 뮤테이션 정의
  - [ ] CREATE, UPDATE, DELETE 뮤테이션
  - [ ] 타입은 types.ts에서만 임포트
- [ ] **graphql/index.ts** - 쿼리/뮤테이션만 export (타입 금지)

#### 3단계: Apollo Hooks

- [ ] **hooks/use-[entity].ts** - Apollo Hooks
  - [ ] useQuery Hooks (목록, 단일)
  - [ ] useMutation Hooks (생성, 수정, 삭제)
  - [ ] fetchPolicy 설정 ("cache-and-network" 권장)
  - [ ] refetchQueries 설정 (캐시 무효화)

#### 4단계: 폼 설정 (config 폴더)

- [ ] **config/[entity]-form-config.ts** - 폼 설정 및 Zod 스키마
  - [ ] 폼 데이터 타입 정의 (z.object)
  - [ ] 필드 힌트 정의 (fieldHints)
  - [ ] DynamicFormConfig 작성 (sections, fields)
  - [ ] 기본값 설정 (defaultValues)
  - [ ] 필드 타입 설정 (text, email, select, password 등)
  - [ ] 조건부 필드 정의 (visible 함수)
  - [ ] 필드 열 구성 설정 (grid: "full" | "half" | "third")

#### 5단계: 컴포넌트 (6개 필수)

- [ ] **components/[entity]-columns.tsx** - 테이블 컬럼 정의
- [ ] **components/[entity]-table.tsx** - 테이블 렌더링
- [ ] **components/[entity]-edit.tsx** - 생성/수정 모달 (Form 컴포넌트 사용)
  - [ ] FormDrawer로 감싸기
  - [ ] Form 컴포넌트 통합 (config 사용)
  - [ ] GraphQL mutation 연결
- [ ] **components/[entity]-header.tsx** - 헤더 (제목, 버튼)
- [ ] **components/[entity]-filter.tsx** - 필터 (QueryFilter 사용)
  - [ ] filterItems 설정 (필터 옵션 정의)
  - [ ] 페이지에서 필요한 props 전달
- [ ] **components/[entity]-stats.tsx** - 통계 카드

#### 6단계: 상태 관리

- [ ] **stores/[entity].store.ts** - UI 상태 (Zustand)
  - [ ] currentPage
  - [ ] itemsPerPage
  - [ ] searchText (UI에 표시할 검색어)
  - [ ] 필요시 추가 UI 상태
- [ ] **services/[entity].service.ts** - Non-React 호출용 (선택)

#### 7단계: Export 및 Page

- [ ] **index.ts** - 공개 API 정의
  - [ ] Components export (config 파일은 index.ts에서 제외)
  - [ ] Hooks export
  - [ ] Types export
  - [ ] Store export
- [ ] **app/(main)/[domain]/[entity]/page.tsx** - List Page
  - [ ] QueryBar 또는 QueryFilter 사용
  - [ ] 쿼리 필터 상태 (useState)
  - [ ] 검색 텍스트 상태 (useState)
  - [ ] GraphQL 변수 변환 (배열 → 쉼표 구분 문자열)
  - [ ] handleClearAllFilters 구현

### GraphQL 쿼리 검증

- [ ] 필요한 필드만 요청하는가?
- [ ] 타입이 types.ts와 일치하는가?
- [ ] 모든 변수에 타입이 지정되어 있는가?
- [ ] 에러 처리가 구현되어 있는가?
- [ ] 캐시 정책이 설정되어 있는가?

### 타입 안전성 검증

- [ ] types.ts에서만 타입을 정의하는가?
- [ ] graphql/에서 타입을 export하지 않는가?
- [ ] 모든 Hook이 types.ts에서 타입을 임포트하는가?
- [ ] `any` 타입을 사용하지 않는가?
- [ ] 응답 타입(Response)을 정의했는가?

---

## 참고 자료

- **내부 문서**: `/docs/IDAM_DATA_FLOW_MERMAID.md` - Mermaid 다이어그램
- [Apollo Client 공식 문서](https://www.apollographql.com/docs/react/)
- [GraphQL 공식 사이트](https://graphql.org/)
- [Next.js 공식 문서](https://nextjs.org/docs)
- [Zustand 공식 문서](https://github.com/pmndrs/zustand)

---

## 최근 업데이트 요약

### v4.4 (2024년 11월 20일) - 폼 필드 레이아웃 & 비밀번호 관리 개선

**주요 개선사항:**

1. **폼 필드 열 구성 (Grid Layout) 지원**
   - FormFieldConfig에 `grid` 속성 추가: `"full" | "half" | "third"`
   - `form.tsx`에서 Tailwind grid-cols-12를 활용한 동적 레이아웃
   - 각 필드별로 독립적인 너비 설정 가능:
     ```typescript
     // "full" → col-span-12 (한 줄 전체)
     // "half" → col-span-6  (반 줄, 기본값)
     // "third" → col-span-4 (1/3 줄)
     ```
   - **예시**: fullName은 "full"로, username과 email은 "half"로 나란히 배치

2. **백엔드 기반 비밀번호 생성으로 전환**
   - **CreateUserInput**: password 필드 완전 제거
   - **폼 설정**: password 입력 필드 삭제
   - **동작**:
     - 생성 모드: 관리자가 비밀번호를 입력하지 않음
     - 백엔드: 임시 비밀번호 자동 생성
     - 사용자: 이메일로 임시 비밀번호 수신
   - **비밀번호 변경**: 별도의 "비밀번호 재설정" 프로세스 사용 (향후 구현)

3. **상태 필드 확장**
   - `status` enum에 "SUSPENDED" 추가: `["ACTIVE", "INACTIVE", "LOCKED", "SUSPENDED"]`
   - User 타입의 userType도 확장: `"ADMIN" | "MANAGER" | "USER" | "MASTER" | "TENANT" | "SYSTEM" | string`

4. **폼 컴포넌트 타입 안정성 개선**
   - `form.tsx`: React Hook Form의 제네릭 타입 충돌 해결
   - `Record<string, any>` 기반 타입 사용으로 TypeScript 호환성 강화
   - `form-field.tsx`: Controller render 함수에서 `null` 대신 Fragment 반환

5. **사용자 폼 필드 배치 최적화**
   ```typescript
   // 새로운 배치
   fullName         (grid: "full")        // 한 줄 전체
   username | email (grid: "half")        // 양쪽 반씩
   userType | phone (grid: "half")        // 양쪽 반씩
   department | position (grid: "half")   // 양쪽 반씩
   status           (grid: "half")        // 반씩 (선택사항)
   ```

**파일 변경:**
- 수정: `form.tsx` (grid-cols-12 시스템, 타입 호환성)
- 수정: `form-field.tsx` (null → Fragment 반환)
- 수정: `form-field-config.ts` (DynamicFormConfig 제네릭 변경)
- 수정: `users-form-config.ts` (password 제거, grid 속성 추가, SUSPENDED 추가)
- 수정: `users.types.ts` (CreateUserInput에서 password 제거, userType 확장)
- 문서: 이 문서 (`00_프론트엔드_개발가이드_20241114.md`) 업데이트

**이점:**
- 비밀번호를 관리자가 입력할 필요 없음 (보안 개선)
- 사용자에게 이메일로 임시 비밀번호 자동 전송
- 폼 레이아웃을 선언적으로 제어 가능
- 모든 모듈에서 일관된 폼 필드 배치

---

### v4.3 (2024년 11월 20일) - 폼 시스템 통합 & 아키텍처 개선

**주요 개선사항:**

1. **공통 Form 컴포넌트 시스템 도입**
   - 새 파일: `@/components/features/form.tsx` (메인 폼 컴포넌트)
   - 새 파일: `@/components/features/form-field.tsx` (필드 렌더러)
   - 새 파일: `@/components/features/form-field-config.ts` (타입 정의)
   - 새 파일: `@/components/features/form-hint-icon.tsx` (도움말 아이콘)
   - 새 파일: `@/components/features/form-drawer.tsx` (폼 드로어, 기존 entity-drawer 리네임)
   - 새 파일: `@/components/features/form-buttons.tsx` (폼 버튼, 기존 entity-form-buttons 리네임)
   - **목표**: 모든 폼에서 일관된 UI 및 동작 보장

2. **폼 컴포넌트 통합 (Form 컴포넌트 기반)**
   - **users 모듈**:
     - 새 파일: `config/users-form-config.ts` (폼 설정 분리)
     - 수정: `components/users-edit.tsx` (Form 컴포넌트 직접 사용)
     - 삭제: `components/users-form.tsx` (불필요 - edit에 통합됨)
   - **roles 모듈**:
     - 새 파일: `config/roles-form-config.ts` (폼 설정 분리)
     - 수정: `components/roles-edit.tsx` (Form 컴포넌트 직접 사용)
     - 삭제: `components/roles-form.tsx` (불필요 - edit에 통합됨)

3. **코드 감소 및 유지보수성 개선**
   - 폼 컴포넌트 코드: ~340줄 → ~105줄 (70% 감소)
   - 중복 코드 제거: HintIcon 통합, FormButtons 통합
   - TypeScript 타입 안전성: FormFieldConfig로 모든 필드 타입화
   - 필요한 컴포넌트 수: 7개 → 6개 (form 통합)

4. **폼 설정 구조 (Declarative Configuration)**
   ```typescript
   // config/[entity]-form-config.ts
   export const userFormConfig: DynamicFormConfig<UserFormData> = {
     schema: userFormSchema,  // Zod 스키마
     defaultValues: { ... },
     sections: [
       {
         id: "section-id",
         title: "섹션 제목",
         fields: [
           {
             name: "fieldName",
             label: "필드 라벨",
             type: "text|email|select|password|...",
             hint: "도움말",
             required: true,
             options: [...],  // select 타입용
             visible: (data) => boolean,  // 조건부 표시
           }
         ]
       }
     ]
   };
   ```

5. **[entity]-edit.tsx 새로운 패턴**
   ```typescript
   // 폼 로직만 담당, UI는 Form 컴포넌트가 자동 생성
   <FormDrawer title={isEditing ? "수정" : "등록"}>
     <Form
       config={userFormConfig}
       initialData={editingUser}
       onSubmit={handleSubmit}
       onCancel={closeForm}
       isLoading={isLoading}
     />
   </FormDrawer>
   ```

**파일 변경 요약:**
- 새로 생성: 6개 파일 (form-*.tsx 컴포넌트, users/roles config 파일)
- 수정됨: 4개 파일 (users-edit.tsx, roles-edit.tsx, index.ts 파일들)
- 삭제됨: 2개 파일 (users-form.tsx, roles-form.tsx)
- 리네임됨: 2개 파일 (entity-drawer → form-drawer, entity-form-buttons → form-buttons)

**다음 단계 (남은 작업):**
- 나머지 16개 모듈에 Form 시스템 적용:
  - tenant-roles, tenant-users, tenants, subscriptions, onboardings (5개)
  - workflows, tasks, executions (3개)
  - permissions, api-keys, login-logs, sessions (4개)
  - resources, compliances, policies, invoice, plans, transactions (6개)
- 각 모듈별로 동일한 패턴으로 config 파일 생성 및 edit 통합

---

### v4.2 (2024년 11월 17일) - UI 컴포넌트 스타일 및 레이아웃 조정

**UI 스타일 개선:**

1. **폰트 크기 최적화 (Option A 스케일)**
   - 기존 3단계 → 새로운 7단계 폰트 스케일 (11px-20px)
   - `globals.css`에서 정의:
     ```css
     --font-size-xs: 0.6875rem;   /* 11px */
     --font-size-sm: 0.75rem;     /* 12px */
     --font-size-base: 0.8125rem; /* 13px */
     --font-size-lg: 0.875rem;    /* 14px */
     --font-size-xl: 1rem;        /* 16px */
     --font-size-2xl: 1.125rem;   /* 18px */
     --font-size-3xl: 1.25rem;    /* 20px */
     ```
   - 파일: `app/globals.css`

2. **컴포넌트별 폰트 적용**
   - Button: `text-xs` → `text-base` (더 가독성 좋음)
   - Label: `text-sm` → `text-lg` (폼 라벨)
   - Form 라벨: `text-xs` → `text-sm` (필드 설명)
   - Page 헤더: `text-3xl` → `text-2xl` (더 바란스 있음)
   - Breadcrumb: `text-sm` → `text-xs`
   - 사용자명: `text-sm` → `text-lg`

3. **테마 토글 버튼 수정**
   - Import 경로 오류 수정: `./theme-provider` → `@/lib/theme-provider`
   - 버튼 크기: 36x36 → 32x32 (w-9 h-9 → w-8 h-8)
   - 검색 입력박스: height 36px → 32px (h-9 → h-8)
   - 파일: `components/providers/theme-toggle.tsx`

4. **라이트 테마 버튼 스타일 수정**
   - 버튼 배경색 문제: 라이트 테마에서도 다크 색상으로 렌더링됨
   - 해결: `bg-neutral-950` → `bg-background/50` (CSS 변수 사용)
   - 적용된 variant: `outline`, `secondary`, `destructive`
   - 파일: `components/ui/button.tsx`

**컴포넌트 레이아웃 조정:**

5. **통계 카드 (Stat Card) 높이 조정**
   - 높이: h-32 (128px) → h-28 (112px) (더 콤팩트함)
   - 내부 레이아웃: `justify-center` → `justify-between` (수직 배치)
   - 패딩: p-6 → p-4
   - 구조:
     ```
     상단: 제목 (text-xs)
     중간: 큰 값 (text-xl)
     하단: Trend + 설명
     ```
   - 파일:
     - `components/stats/stats-card.tsx`
     - `features/dashboard/components/stat-card.tsx`

6. **데이터 테이블 헤더 높이 조정**
   - TableHead 높이: h-12 (48px) → h-10 (40px)
   - 패딩: py-3 → py-2
   - 파일:
     - `components/data-table/data-table.tsx`
     - `tenants-web/components/data-table/data-table.tsx`
     - `tenants-web/components/data-table/data-table-advanced.tsx`

**영향받는 파일 (총 29개):**
- UI 컴포넌트: button, label, data-table (3개)
- 레이아웃: main-header, page-header, page, theme-toggle (4개)
- 통계: stats-card, dashboard stat-card (2개)
- 폼: users-form (1개)
- 필터: query-bar (1개)
- 테마: theme-settings, theme-preview, theme-preview-card (3개)
- 인증: signin, signup, logout, forgot-password, reset-password (5개)
- 기타: tenant-growth-chart, resources, permissions, login-logs, plans (7개)

**이점:**
- 일관된 폰트 스케일로 더 세련된 UI
- 대시보드 중심의 폰트 크기 최적화
- 라이트/다크 테마 자동 지원
- 더 효율적인 공간 활용 (컴팩트한 카드, 테이블)

---

### v4.1 (2024년 11월 17일) - 폼 개선 및 토큰 갱신 수정

**버그 수정:**

1. **토큰 갱신 GraphQL 쿼리 오류 수정**
   - 문제: `"badly formed hexadecimal UUID string"` 오류
   - 원인: 토큰 갱신 뮤테이션에서 'auth' 네임스페이스 불필요
   - 수정:
     ```typescript
     // ❌ 변경 전
     auth { refreshToken(input: $input) { ... } }

     // ✅ 변경 후
     refreshToken(input: $input) { ... }
     ```
   - 파일: `lib/apollo-client.ts` (라인 1047)

2. **QueryText 즉시 검색 문제 수정**
   - 문제: 사용자가 타이핑 중일 때 각 키마다 GraphQL 쿼리 실행
   - 해결: 500ms 디바운스 추가
   ```typescript
   // UI 즉시 업데이트
   const [queryText, setQueryTextLocal] = useState("");

   // GraphQL 쿼리는 500ms 후 실행
   const [debouncedQueryText, setDebouncedQueryText] = useState("");
   useEffect(() => {
     const timer = setTimeout(() => {
       setDebouncedQueryText(queryText);
     }, 500);
     return () => clearTimeout(timer);
   }, [queryText]);
   ```
   - 파일: `app/(main)/idam/users/page.tsx` (라인 582-594)

**기능 개선:**

3. **Edit 모드 연결 구현**
   - 테이블의 Edit 버튼 → selectedId 저장 → 폼 데이터 로드
   - 구현:
     ```typescript
     // Page에서
     const handleEdit = (user: User) => {
       setSelectedId(user.id);
       openForm();
     };

     // Zustand Store에
     setSelectedId: (id) => set({ selectedId: id })
     ```
   - 파일: `app/(main)/idam/users/page.tsx` (라인 640-646)
   - 파일: `stores/users.store.ts` (라인 1257)

4. **폼 데이터 로드 개선**
   - 생성 모드: 모든 필드 초기화
   - 수정 모드: 기존 데이터 로드 + username 비활성화
   - useEffect에서 조건부 reset() 호출:
   ```typescript
   useEffect(() => {
     if (initialData) {
       // 수정 모드: 기존 데이터 로드
       reset({ ...initialData });
     } else {
       // 생성 모드: 초기화
       reset({ ...initialValues });
     }
   }, [initialData, reset]);
   ```
   - 파일: `components/users-form.tsx` (라인 563-592)

5. **Hint Icons 구현**
   - 각 폼 필드에 물음표 아이콘 추가
   - 클릭 시 Popover로 도움말 표시
   - 라벨 우측에 정렬 (`justify-between`)
   - 파일: `components/users-form.tsx` (라인 495-517)

**영향받는 파일:**
- `app/(main)/idam/users/page.tsx` (debounce, edit handler 추가)
- `components/users-form.tsx` (HintIcon, form reset 개선)
- `stores/users.store.ts` (setSelectedId, openForm 추가)
- `lib/apollo-client.ts` (토큰 갱신 쿼리 수정)

---

### v4.0 (2024년 11월 17일) - Query 필터링 & 필터 UI 완성

**새로 추가된 기능:**

1. **Query 기반 필터링 아키텍처**
   - Jira 스타일 필터링: 자유 검색 텍스트 + 구조화된 필터
   - 필터 팝업의 임시 상태 관리 (tempQueryFilters)
   - 실시간 필터 개수 표시 및 배지 업데이트

2. **공통 필터 컴포넌트** (`@/components/filters/`)
   - `QueryBar`: 검색 입력 + 필터 버튼 UI
   - `QueryFilter`: QueryBar + QueryFilterPopup 복합 컴포넌트
   - `QueryFilterPopup`: Jira 스타일 좌우 분할 필터 팝업
   - `QueryCheckboxGroup`: 멀티 선택 체크박스

3. **필터 UI 개선**
   - 필터 지우기 버튼 (X 아이콘 + 텍스트)
   - 활성 필터 개수 실시간 표시
   - 화면 깜빡임 제거 (페이지 초기화 추가)

4. **상태 관리 개선**
   - UI 상태 (페이지, 크기)와 쿼리 상태 (필터) 분리
   - 팝업의 임시 상태로 서버 쿼리 즉시 트리거 방지
   - 필터 초기화 시 페이지 번호도 함께 초기화

---

**버전**: 4.4 (폼 필드 레이아웃 & 비밀번호 관리 개선)
**마지막 업데이트**: 2024년 11월 20일
**상태**: ✅ 프로덕션 준비 완료 (Users, Roles 모듈 기준)
