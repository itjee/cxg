...
(
    ...
    -- 제약조건
    CONSTRAINT uk_sys_users__username UNIQUE (username),
    CONSTRAINT uk_sys_users__email UNIQUE (email),
    CONSTRAINT uk_sys_users__user_code UNIQUE (user_code)
);
...
CREATE TABLE sys.roles (
    ...
    CONSTRAINT uk_sys_roles__code UNIQUE (role_code)
);
...
COMMENT ON INDEX idx_sys_users_id IS '테넌트별 사용자 조회 최적화';
COMMENT ON INDEX idx_sys_users_active IS '활성 사용자 조회 최적화 (Partial Index)';
COMMENT ON INDEX idx_sys_users_email IS '이메일 검색 인덱스';

COMMENT ON INDEX idx_sys_roles_id IS '테넌트별 역할 조회 최적화';
COMMENT ON INDEX idx_sys_roles_active IS '활성 역할 조회 최적화';
...
-- 테이블 생성
CREATE TABLE IF NOT EXISTS sys.sys_code_rules (
    -- 기본키
