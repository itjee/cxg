...
(
	...
	CONSTRAINT ck_companies__company_code 	CHECK (company_code ~ '^[A-Z0-9_]{2,50}$'),
...
COMMENT ON COLUMN adm.companies.tenant_id           IS '테넌트 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_companies__tenant_id
    ON adm.companies (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_companies__company_code
    ON adm.companies (company_code)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_companies__company_name
...
-- 활성 상태 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_companies__status
    ON adm.companies (status, tenant_id)
 WHERE is_deleted = false;
...
-- 테넌트별 회사 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_companies__tenant_code
    ON adm.companies (tenant_id, company_code)
 WHERE is_deleted = false;
...
COMMENT ON COLUMN adm.departments.tenant_id        IS '테넌트 식별자';
COMMENT ON COLUMN adm.departments.company_id       IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_departments__company_id
    ON adm.departments (company_id)
 WHERE is_deleted = false;
 
CREATE INDEX IF NOT EXISTS ix_departments__tenant_id
    ON adm.departments (tenant_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_departments__department_type
    ON adm.departments (department_type, company_id)
 WHERE is_deleted = false;

-- 정렬 순서 인덱스
CREATE INDEX IF NOT EXISTS ix_departments__sort_order
    ON adm.departments (company_id, parent_id, sort_order)
 WHERE is_deleted = false;

-- 활성 상태 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_departments__status
    ON adm.departments (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 부서 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_departments__company_code
    ON adm.departments (company_id, department_code)
 WHERE is_deleted = false;

-- 회사별 부서명 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_departments__company_name
    ON adm.departments (company_id, department_name)
 WHERE is_deleted = false;
...
COMMENT ON COLUMN adm.employees.tenant_id       IS '테넌트 식별자';
COMMENT ON COLUMN adm.employees.company_id      IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_employees__tenant_id
    ON adm.employees (tenant_id)
 WHERE is_deleted = false;
 
CREATE INDEX IF NOT EXISTS ix_employees__company_id
    ON adm.employees (company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_employees__work_type
    ON adm.employees (work_type, company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_employees__status
    ON adm.employees (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 사원번호 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_employees__company_employee_code
    ON adm.employees (company_id, employee_code)
 WHERE is_deleted = false;
...
-- departments
ALTER TABLE adm.departments ADD CONSTRAINT fk_departments__company_id   FOREIGN KEY (company_id) 	REFERENCES adm.companies(id)	ON DELETE CASCADE;
...
-- employees
ALTER TABLE adm.employees 	ADD CONSTRAINT fk_employees__company_id     FOREIGN KEY (company_id) 	REFERENCES adm.companies(id)	ON DELETE CASCADE;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_customers__company_id     FOREIGN KEY (company_id) REFERENCES adm.companies(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN adm.customers.tenant_id           IS '테넌트 식별자';
COMMENT ON COLUMN adm.customers.company_id          IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customers__tenant_id
    ON adm.customers (tenant_id)
 WHERE is_deleted = false;
 
CREATE INDEX IF NOT EXISTS ix_customers__company_id
    ON adm.customers (company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_customers__customer_type
    ON adm.customers (customer_type, company_id)
 WHERE is_deleted = false;
...
-- 활성 상태 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customers__status
    ON adm.customers (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 거래처 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_customers__company_code
    ON adm.customers (company_id, customer_code)
 WHERE is_deleted = false;
...
COMMENT ON COLUMN adm.customer_contacts.tenant_id    	IS '테넌트 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_contacts__tenant_id
    ON adm.customer_contacts (tenant_id)
 WHERE is_deleted = false;
...
-- 활성 상태 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_contacts__status
    ON adm.customer_contacts (status, customer_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_customer_managers__company_id     FOREIGN KEY (company_id) 	REFERENCES adm.companies(id)	ON DELETE CASCADE,
...
COMMENT ON COLUMN adm.customer_managers.tenant_id     	IS '테넌트 식별자';
COMMENT ON COLUMN adm.customer_managers.company_id     	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_managers__tenant_id
    ON adm.customer_managers (tenant_id)
 WHERE is_deleted = false;
 
CREATE INDEX IF NOT EXISTS ix_customer_managers__company_id
    ON adm.customer_managers (company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_managers__status
    ON adm.customer_managers (status, company_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_customer_banks__company_id    FOREIGN KEY (company_id) 	REFERENCES adm.companies(id)	ON DELETE CASCADE,
...
COMMENT ON COLUMN adm.customer_banks.tenant_id     	IS '테넌트 식별자';
COMMENT ON COLUMN adm.customer_banks.company_id    	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_banks__tenant_id
    ON adm.customer_banks (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_customer_banks__company_id
    ON adm.customer_banks (company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_banks__status
    ON adm.customer_banks (status, customer_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_customer_addresses__company_id    FOREIGN KEY (company_id) REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.customer_addresses.tenant_id     	IS '테넌트 식별자';
COMMENT ON COLUMN adm.customer_addresses.company_id    	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_addresses__tenant_id
    ON adm.customer_addresses (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_customer_addresses__company_id
    ON adm.customer_addresses (company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_customer_addresses__status
    ON adm.customer_addresses (status, customer_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_adm_makers__company_id     	FOREIGN KEY (company_id) REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.makers.company_id     IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_adm_makers__company_id
    ON adm.makers (company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_adm_makers__display_order
    ON adm.makers (company_id, display_order, maker_name)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_adm_makers__status
    ON adm.makers (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 제조사 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_adm_makers__company_code
    ON adm.makers (company_id, maker_code)
 WHERE is_deleted = false;

-- 회사별 제조사명 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_adm_makers__company_name
    ON adm.makers (company_id, maker_name)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_adm_brands__company_id        FOREIGN KEY (company_id) 	REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.brands.company_id    	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_adm_brands__company_id
    ON adm.brands (company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_adm_brands__brand_type
    ON adm.brands (brand_type, company_id)
 WHERE is_deleted = false;
...
-- 프리미엄 브랜드 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_adm_brands__is_premium
    ON adm.brands (company_id)
 WHERE is_premium = true 
   AND is_deleted = false;

-- 자체 브랜드 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_adm_brands__is_private
    ON adm.brands (company_id)
 WHERE is_private = true 
   AND is_deleted = false;

-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_adm_brands__status
    ON adm.brands (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 브랜드 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_adm_brands__company_code
    ON adm.brands (company_id, brand_code)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_categories__company_id   FOREIGN KEY (company_id) REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.categories.tenant_id     	IS '테넌트 식별자';
COMMENT ON COLUMN adm.categories.company_id    	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_categories__tenant_id
    ON adm.categories  (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_categories__company_id
    ON adm.categories  (company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_categories__level_depth
    ON adm.categories  (company_id, level_depth, display_order)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_categories__category_type
    ON adm.categories  (category_type, company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_categories__status
    ON adm.categories  (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 카테고리 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_categories__company_code
    ON adm.categories  (company_id, category_code)
 WHERE is_deleted = false;

-- 회사별 동일 레벨의 카테고리명 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_categories__company_parent_name
    ON adm.categories  (company_id, COALESCE(parent_id, '00000000-0000-0000-0000-000000000000'::UUID), category_name)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_category_managers__company_id     FOREIGN KEY (company_id) 	REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.category_managers.tenant_id     	IS '테넌트 식별자';
COMMENT ON COLUMN adm.category_managers.company_id     	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_category_managers__tenant_id
    ON adm.category_managers (tenant_id)
 WHERE is_deleted = false;
 
CREATE INDEX IF NOT EXISTS ix_category_managers__company_id
    ON adm.category_managers (company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_category_managers__status
    ON adm.category_managers (status, company_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_products__company_id      FOREIGN KEY (company_id) 	REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.products.tenant_id        IS '테넌트 식별자';
COMMENT ON COLUMN adm.products.company_id       IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_products__tenant_id
    ON adm.products (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_products__company_id
    ON adm.products (company_id)
 WHERE is_deleted = false;
...
-- 특성별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_products__is_serial
    ON adm.products (company_id)
 WHERE is_serial = true 
   AND is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_products__is_taxfree
    ON adm.products (company_id)
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_products__status
    ON adm.products (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 제품 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_products__company_code
    ON adm.products (company_id, product_code)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_product_managers__company_id  	FOREIGN KEY (company_id) 	REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.product_managers.tenant_id       	IS '테넌트 식별자';
COMMENT ON COLUMN adm.product_managers.company_id      	IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_product_managers__tenant_id
    ON adm.product_managers (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_product_managers__company_id
    ON adm.product_managers (company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_product_managers__status
    ON adm.product_managers (status, company_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_warehouses__company_id    FOREIGN KEY (company_id) REFERENCES adm.companies(id)	ON DELETE CASCADE,
...
COMMENT ON COLUMN adm.warehouses.tenant_id     			IS '테넌트 식별자';
COMMENT ON COLUMN adm.warehouses.company_id    			IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouses__tenant_id
    ON adm.warehouses (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_warehouses__company_id
    ON adm.warehouses (company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_warehouses__warehouse_type
    ON adm.warehouses (warehouse_type, company_id)
 WHERE is_deleted = false;
...
CREATE INDEX IF NOT EXISTS ix_warehouses__is_primary
    ON adm.warehouses (company_id, is_primary)
 WHERE is_primary = true 
   AND is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouses__status
    ON adm.warehouses (status, company_id)
 WHERE is_deleted = false;
...
-- 회사별 창고 코드 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_warehouses__company_code
    ON adm.warehouses (company_id, warehouse_code)
 WHERE is_deleted = false;

-- 회사별 창고명 유니크
CREATE UNIQUE INDEX IF NOT EXISTS ux_warehouses__company_name
    ON adm.warehouses (company_id, warehouse_name)
 WHERE is_deleted = false;

-- 회사별 본창고는 하나만
CREATE UNIQUE INDEX IF NOT EXISTS ux_warehouses__company_main
    ON adm.warehouses (company_id)
 WHERE is_primary = true 
   AND is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_warehouse_employees__company_id   FOREIGN KEY (company_id) REFERENCES adm.companies(id),
...
COMMENT ON COLUMN adm.warehouse_employees.tenant_id    		IS '테넌트 식별자';
COMMENT ON COLUMN adm.warehouse_employees.company_id   		IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouse_employees__tenant_id
    ON adm.warehouse_employees (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_warehouse_employees__company_id
    ON adm.warehouse_employees (company_id)
 WHERE is_deleted = false;
...
-- 창고별 사원 목록 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouse_employees__warehouse_active
    ON adm.warehouse_employees (warehouse_id, status, role_type)
 WHERE is_deleted = false;

-- 사원별 창고 목록 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouse_employees__employee_active
    ON adm.warehouse_employees (employee_id, status, start_date DESC)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouse_employees__status
    ON adm.warehouse_employees (status, company_id)
 WHERE is_deleted = false;
...
(
	...
	-- 외래키 제약 조건
    CONSTRAINT fk_warehouse_locations__company_id       FOREIGN KEY (company_id) 	REFERENCES adm.companies(id)			ON DELETE CASCADE,
...
COMMENT ON COLUMN adm.warehouse_locations.tenant_id    		IS '테넌트 식별자';
COMMENT ON COLUMN adm.warehouse_locations.company_id   		IS '회사 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouse_locations__tenant_id
    ON adm.warehouse_locations (tenant_id)
 WHERE is_deleted = false;

CREATE INDEX IF NOT EXISTS ix_warehouse_locations__company_id
    ON adm.warehouse_locations (company_id)
 WHERE is_deleted = false;
...
-- 상태별 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_warehouse_locations__status
    ON adm.warehouse_locations (status, warehouse_id)
 WHERE is_deleted = false;
