...
CREATE INDEX IF NOT EXISTS ix_quotations__company_id ON srm.quotations (company_id) WHERE deleted = false;
CREATE INDEX IF NOT EXISTS ix_quotation_lines__quote_id ON srm.quotation_lines (quote_id);
CREATE INDEX IF NOT EXISTS ix_sales_orders__company_id ON srm.sales_orders (company_id) WHERE deleted = false;
CREATE INDEX IF NOT EXISTS ix_sales_order_lines__so_id ON srm.sales_order_lines (so_id);

CREATE UNIQUE INDEX IF NOT EXISTS ux_quotations__company_code ON srm.quotations (company_id, quote_code) WHERE deleted = false;
CREATE UNIQUE INDEX IF NOT EXISTS ux_quotation_lines__quote_line ON srm.quotation_lines (quote_id, line_no);
CREATE UNIQUE INDEX IF NOT EXISTS ux_sales_orders__company_code ON srm.sales_orders (company_id, so_code) WHERE deleted = false;
