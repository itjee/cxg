...
COMMENT ON COLUMN asm.service_requests.tenant_id           IS '테넌트 식별자';
...
-- 기본 조회 인덱스
CREATE INDEX IF NOT EXISTS ix_service_requests__tenant_id
    ON asm.service_requests (tenant_id)
...
-- 인덱스
CREATE INDEX IF NOT EXISTS idx_asm_service_requests_tenant_id ON asm.service_requests(tenant_id);
...
COMMENT ON COLUMN asm.service_requests.id IS 'A/S 요청 고유 식별자';
COMMENT ON COLUMN asm.service_requests.tenant_id IS '테넌트 ID';
