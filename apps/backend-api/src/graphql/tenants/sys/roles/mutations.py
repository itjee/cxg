...
async def create_tenant_role(
    db: AsyncSession,
    input_data: TenantRoleCreateInput,
    created_by: UUID
) -> TenantRole:
    """Tenant 역할 생성"""
...
async def update_tenant_role(
    db: AsyncSession,
    role_id: UUID,
    input_data: TenantRoleUpdateInput,
    updated_by: UUID
) -> Optional[TenantRole]:
    """Tenant 역할 수정"""
...
class TenantRoleMutations:
    ...
    async def create_tenant_role(
        ...
        return await create_tenant_role(db, input, current_user_id)
    ...
    async def update_tenant_role(
        ...
        current_user_id = UUID(info.context.user_id)
        return await update_tenant_role(db, UUID(id), input, current_user_id)
