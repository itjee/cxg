...
async def resolve_manager_user_role_user(user_id: UUID, info):
    """
    ...
    if loader:
        ...
        if user_model:
            ...
            return manager_user_to_graphql(user_model)
        return None
...
async def resolve_manager_user_role_role(role_id: UUID, info):
    """
    사용자-역할 매핑의 역할 정보 조회
...
async def resolve_manager_granted_by_user(granted_by_id: UUID, info):
    """
    역할 부여자 정보 조회
    ...
    return await resolve_manager_user_role_user(granted_by_id, info)
