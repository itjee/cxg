...
from src.models.manager.tnnt import Subscription
...
class SubscriptionService:
    ...
    @staticmethod
    async def create(
        db: AsyncSession, data: SubscriptionCreate, created_by: UUID | None = None
    ) -> SubscriptionResponse:
        """Create a new Subscription."""
        obj = Subscription(**data.model_dump(), created_by=created_by)
        db.add(obj)
        await db.commit()
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, obj_id: UUID) -> SubscriptionResponse:
        """Get Subscription by ID."""
        stmt = select(Subscription).where(Subscription.id == obj_id, Subscription.deleted == False)
    ...
    @staticmethod
    async def update(
        db: AsyncSession, obj_id: UUID, data: SubscriptionUpdate, updated_by: UUID | None = None
    ) -> SubscriptionResponse:
        """Update Subscription."""
        stmt = select(Subscription).where(Subscription.id == obj_id, Subscription.deleted == False)
        result = await db.execute(stmt)
