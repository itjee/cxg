...
async def create(
    ...
    obj = await TasksService.create(db, data, created_by=creator_id)
...
async def get_list(
    ...
    """Tasks 목록 조회."""
    result = await TasksService.get_list(db, page=page, page_size=page_size, search=search)
...
@router.get(
    ...
    response_model=EnvelopeResponse[TasksResponse],
    summary="Tasks 상세 조회",
    description="Tasks 상세 정보를 조회합니다.",
)
async def get_detail(
    ...
    """Tasks 상세 조회."""
    obj = await TasksService.get_by_id(db, obj_id)
    return EnvelopeResponse.success_response(obj)
...
async def update(
    ...
    obj = await TasksService.update(db, obj_id, data, updated_by=updater_id)
    return EnvelopeResponse.success_response(obj)
...
@router.delete(
    ...
    summary="Tasks 삭제",
    description="Tasks를 삭제합니다 (소프트 삭제).",
)
async def delete(
    obj_id: UUID,
    db: AsyncSession = Depends(get_manager_db),
    ...
    await TasksService.delete(db, obj_id, deleted_by=deleter_id)
    return EnvelopeResponse.success_response({"message": "Tasks가 삭제되었습니다"})