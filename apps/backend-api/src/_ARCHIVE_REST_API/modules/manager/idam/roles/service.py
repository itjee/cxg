...
from src.models.manager.idam import Role
...
class RoleService:
    ...
    @staticmethod
    async def create(
        db: AsyncSession, data: RoleCreate, created_by: UUID | None = None
    ) -> RoleResponse:
        """Create a new Role."""
        obj = Role(**data.model_dump(), created_by=created_by)
        db.add(obj)
        await db.commit()
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, obj_id: UUID) -> RoleResponse:
        """Get Role by ID."""
        stmt = select(Role).where(Role.id == obj_id, Role.is_active == True)
    ...
    @staticmethod
    async def update(
        db: AsyncSession, obj_id: UUID, data: RoleUpdate, updated_by: UUID | None = None
    ) -> RoleResponse:
        """Update Role."""
        stmt = select(Role).where(Role.id == obj_id, Role.is_active == True)
        result = await db.execute(stmt)
