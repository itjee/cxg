...
import { Permissions as PermissionsIcon, UserCheck, UserX, Shield } from "lucide-react";
...
export function PermissionsStats({ permissions }: PermissionsStatsProps) {
  ...
  const active = permissions.filter((u) => u.is_active).length;
  const inactive = total - active;
  const admins = permissions.filter((u) => u.is_active).length;

  const stats = [
    { label: "전체 permission", value: total, icon: PermissionsIcon, color: "text-blue-600", bgColor: "bg-blue-100" },
