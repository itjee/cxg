/**
 * Login Logs Zustand Store
 *
 * UI 상태만 관리합니다.
 * 서버 상태(데이터)는 Apollo Client가 관리합니다.
 *
 * @example
 * const { currentPage, itemsPerPage, formOpen } = useLoginLogsStore();
 */

import { create } from "zustand";
import type { Updater } from "@tanstack/react-table";

interface LoginLogsStore {
  // ===== UI 상태 =====
  currentPage: number;
  itemsPerPage: number;
  searchText: string;
...
export const useLoginLogsStore = create<LoginLogsStore>((set) => ({
  ...initialState,
