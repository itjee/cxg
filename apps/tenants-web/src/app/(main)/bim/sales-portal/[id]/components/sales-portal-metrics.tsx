...
import { TrendingUp, TrendingDown, Target, Award, Clock, Percent } from 'lucide-react';
...
interface SalesPortalMetricsProps {
  performanceData: {
...
export function SalesPortalMetrics({ performanceData }: SalesPortalMetricsProps) {
  ...
  const metrics: MetricCard[] = [
    {
      label: '파이프라인 총액',
      value: formatCurrency(performanceData.pipelineTotal),
      unit: '',
      trend: 'up',
      change: 12.5,
    ...
    {
      label: '할당량 달성률',
      value: performanceData.quotaAchievementRate,
      unit: '%',
      trend: 'up',
      change: 5.2,
    ...
    {
      label: '평균 거래 규모',
      value: formatCurrency(performanceData.avgDealSize),
      unit: '',
      trend: 'stable',
    ...
    {
      label: '평균 성약 일수',
      value: performanceData.daysToClose,
      unit: '일',
      trend: 'down',
      change: -2.4,
    ...
    {
      label: '이번 달 활동',
      value: `${performanceData.thisMonthCalls + performanceData.thisMonthEmails + performanceData.thisMonthMeetings}`,
      unit: '건',
      trend: 'up',
      change: 8.7,
      icon: <TrendingUp className="w-full h-full" />,
              ...
              <div className="relative z-10">
                ...
                {metric.trend && (
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground ml-auto">지난달 대비</span>
                  </div>
                )}
              </div>
            </CardContent>
