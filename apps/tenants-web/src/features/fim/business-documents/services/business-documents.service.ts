...
interface BusinessDocumentsServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 BusinessDocumentsServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as BusinessDocumentsServiceError;
  err.message = `[BusinessDocumentsService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/business-documents';

export const businessDocumentsService = {
