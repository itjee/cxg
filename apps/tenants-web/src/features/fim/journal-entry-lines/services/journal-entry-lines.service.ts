/**
 * JournalEntryLines Service
 * API를 통한 분개 라인 데이터 관리
 */
...
interface JournalEntryLinesServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 JournalEntryLinesServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as JournalEntryLinesServiceError;
  err.message = `[JournalEntryLinesService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/journal-entry-lines';

export const journalEntryLinesService = {
