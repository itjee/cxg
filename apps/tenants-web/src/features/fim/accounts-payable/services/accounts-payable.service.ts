...
interface AccountsPayableServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 AccountsPayableServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as AccountsPayableServiceError;
  err.message = `[AccountsPayableService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/accounts-payable';
