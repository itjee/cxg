...
interface JournalEntriesServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 JournalEntriesServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as JournalEntriesServiceError;
  err.message = `[JournalEntriesService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/journal-entries';
