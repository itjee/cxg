...
interface PaymentTransactionsServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 PaymentTransactionsServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as PaymentTransactionsServiceError;
  err.message = `[PaymentTransactionsService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/payment-transactions';
