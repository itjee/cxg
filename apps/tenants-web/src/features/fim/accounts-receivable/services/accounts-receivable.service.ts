...
interface AccountsReceivableServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 AccountsReceivableServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as AccountsReceivableServiceError;
  err.message = `[AccountsReceivableService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/accounts-receivable';
