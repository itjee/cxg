/**
 * TaxInvoiceLines Service
 * API를 통한 세금계산서 라인 데이터 관리
 */
...
interface TaxInvoiceLinesServiceError extends Error {
  code?: string;
  status?: number;
}

/**
 * API 에러를 TaxInvoiceLinesServiceError로 변환
 */
function handleApiError(error: unknown, context: string): never {
  const err = error as TaxInvoiceLinesServiceError;
  err.message = `[TaxInvoiceLinesService] ${context}: ${err.message || 'Unknown error'}`;
  throw err;
}

const ENDPOINT = '/fim/tax-invoice-lines';

export const taxInvoiceLinesService = {
