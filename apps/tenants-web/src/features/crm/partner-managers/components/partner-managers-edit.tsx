...
interface PartnerManagersEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function PartnerManagersEdit({ data, onSubmit }: PartnerManagersEditProps) {
  const { formOpen, editingId, closeForm } = usePartnerManagersStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '거래처 담당자 수정' : '거래처 담당자 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '거래처 담당자 정보를 수정하세요.' : '새로운 거래처 담당자 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
