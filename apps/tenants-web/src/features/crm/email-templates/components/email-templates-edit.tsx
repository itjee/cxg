...
interface EmailTemplatesEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function EmailTemplatesEdit({ data, onSubmit }: EmailTemplatesEditProps) {
  const { formOpen, editingId, closeForm } = useEmailTemplatesStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '이메일 템플릿 수정' : '이메일 템플릿 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '이메일 템플릿 정보를 수정하세요.' : '새로운 이메일 템플릿 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
