...
interface PartnerAddressesEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function PartnerAddressesEdit({ data, onSubmit }: PartnerAddressesEditProps) {
  const { formOpen, editingId, closeForm } = usePartnerAddressesStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '거래처 주소 수정' : '거래처 주소 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '거래처 주소 정보를 수정하세요.' : '새로운 거래처 주소 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
