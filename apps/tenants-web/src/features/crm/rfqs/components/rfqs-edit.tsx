...
interface RfqsEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function RfqsEdit({ data, onSubmit }: RfqsEditProps) {
  const { formOpen, editingId, closeForm } = useRfqsStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '견적요청 수정' : '견적요청 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '견적요청 정보를 수정하세요.' : '새로운 견적요청 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
