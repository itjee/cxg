...
interface CustomerSegmentsEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function CustomerSegmentsEdit({ data, onSubmit }: CustomerSegmentsEditProps) {
  const { formOpen, editingId, closeForm } = useCustomerSegmentsStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '고객 세그먼트 수정' : '고객 세그먼트 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '고객 세그먼트 정보를 수정하세요.' : '새로운 고객 세그먼트 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
