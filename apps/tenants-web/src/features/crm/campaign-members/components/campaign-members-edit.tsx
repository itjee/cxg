...
interface CampaignMembersEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function CampaignMembersEdit({ data, onSubmit }: CampaignMembersEditProps) {
  const { formOpen, editingId, closeForm } = useCampaignMembersStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '캠페인 회원 수정' : '캠페인 회원 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '캠페인 회원 정보를 수정하세요.' : '새로운 캠페인 회원 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
