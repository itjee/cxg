...
interface CustomerSurveysEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function CustomerSurveysEdit({ data, onSubmit }: CustomerSurveysEditProps) {
  const { formOpen, editingId, closeForm } = useCustomerSurveysStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '고객 설문 수정' : '고객 설문 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '고객 설문 정보를 수정하세요.' : '새로운 고객 설문 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
