...
interface CampaignsEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function CampaignsEdit({ data, onSubmit }: CampaignsEditProps) {
  const { formOpen, editingId, closeForm } = useCampaignsStore();
    ...
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          ...
          <SheetDescription>
            {editingId ? '캠페인 정보를 수정하세요.' : '새로운 캠페인 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
