...
interface InteractionsEditProps {
  data: any[];
  onSubmit: (formData: Record<string, any>) => void;
}

export function InteractionsEdit({ data, onSubmit }: InteractionsEditProps) {
  const { formOpen, editingId, closeForm } = useInteractionsStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '상호작용 수정' : '상호작용 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '상호작용 정보를 수정하세요.' : '새로운 상호작용 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
