...
export function LeadsEdit({ data, onSubmit }: LeadsEditProps) {
  const { formOpen, editingId, closeForm } = useLeadsStore();
  ...
  return (
    <Sheet open={formOpen} onOpenChange={(open) => !open && closeForm()}>
      <SheetContent className="overflow-y-auto w-[600px]">
        <SheetHeader>
          <SheetTitle>
            {editingId ? '리드 수정' : '리드 등록'}
          </SheetTitle>
          <SheetDescription>
            {editingId ? '리드 정보를 수정하세요.' : '새로운 리드 정보를 입력하세요.'}
          </SheetDescription>
        </SheetHeader>
