...
import { Edit2, Trash2, ArrowUpDown, Package } from 'lucide-react';
...
export function CategoriesTable({
  ...
  const columns: ColumnDef<Category>[] = [
    ...
    },
    {
      ...
      header: '상위 카테고리',
      cell: ({ row }) => (
        <div className="text-sm text-muted-foreground">{row.getValue('parent_category') || '-'}</div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: '표시 순서',
      cell: ({ row }) => <div className="text-sm font-medium">{row.getValue('display_order')}</div>,
    },
    {
