...
interface ApprovalRequestsServiceError extends Error {
  code?: string;
  status?: number;
}
...
const handleApiError = (error: any, context: string) => {
  const err: ApprovalRequestsServiceError = new Error(
  ...
  err.name = 'ApprovalRequestsServiceError';

  console.error(`[ApprovalRequestsService] ${context}:`, {
    message: err.message,
    code: err.code,
    status: err.status,
  });
...
const ENDPOINT = '/apm/approval-requests';
